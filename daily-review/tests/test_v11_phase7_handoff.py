...
def test_handoff_generates_unique_package_metadata_output_and_copy(
    ...
    monkeypatch.setattr(
        cli.subprocess,
        "run",
        lambda *args, **kwargs: copied.setdefault("text", kwargs["input"])
...
def test_receive_valid_markdown_saves_inbox_draft_and_handoff_state(tmp_path):
    ...
    result = runner.invoke(
        app, ["receive", "--json-text", content, "--root", str(tmp_path)]
    )
    assert result.exit_code == 0, result.output
    inbox = json.loads(
        (tmp_path / "data" / "inbox" / f"{DAY}.json").read_text(encoding="utf-8")
    )
    handoffs = json.loads(
...
def test_receive_expiry_allow_expired_and_force_backup(tmp_path):
    ...
    allowed = runner.invoke(
        app, _receive_args(tmp_path, _payload(item), "--allow-expired")
    )
    ...
    blocked = runner.invoke(
        app, _receive_args(tmp_path, replacement, "--allow-expired")
    )
    forced = runner.invoke(
        app, _receive_args(tmp_path, replacement, "--allow-expired", "--force")
    )
...
def test_receive_yes_approve_list_cancel_and_home_doctor_release(tmp_path):
    ...
    listing = runner.invoke(
        app, ["handoff-list", "--date", DAY, "--json", "--root", str(tmp_path)]
    )
    assert json.loads(listing.output)["handoffs"][0]["status"] == "approved"
    cannot_cancel = runner.invoke(
        app,
        [
            "handoff-cancel",
            "--date",
...
def test_handoff_list_latest_and_receive_dry_run_do_not_write(tmp_path):
    ...
    latest = runner.invoke(
        app,
        ["handoff-list", "--date", DAY, "--latest", "--json", "--root", str(tmp_path)],
    )
    dry = runner.invoke(app, _receive_args(tmp_path, _payload(second), "--dry-run"))
    assert (
        json.loads(latest.output)["handoffs"][0]["session_id"] == second["session_id"]
    )
...
def test_receive_approve_reuses_existing_review_flow(tmp_path, monkeypatch):
    ...
    result = runner.invoke(
        app, _receive_args(tmp_path, _payload(item), "--approve"), input="n\n"
    )
    assert result.exit_code == 0, result.output
...
def test_receive_yes_rejects_a_non_issued_handoff(tmp_path):
    ...
    unsafe = runner.invoke(
        app,
        _receive_args(
            tmp_path, _payload(item, raw_text="別の回答"), "--force", "--yes"
        ),
    )
