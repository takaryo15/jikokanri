        ...
        "milestones": [
            {
                ...
                "quantitative_metrics": [],
                "steps": [
                    {"id": "step-12345678", "status": "todo", "due_date": due_date}
                ],
            }
        ],
    }
...
def test_handoff_expiry_is_exact_at_0500_jst():
    ...
    assert not is_expired(
        item, now=datetime(2026, 7, 15, 4, 59, 59, tzinfo=ZoneInfo("Asia/Tokyo"))
    )
    assert is_expired(
        item, now=datetime(2026, 7, 15, 5, 0, tzinfo=ZoneInfo("Asia/Tokyo"))
    )


def test_due_date_is_not_overdue_until_the_following_day():
    on_due = _goal_snapshot(
        _goal(),
        period_end="2026-07-31",
        activity={"planned_items": 1, "linked_items": 1},
    )
    after_due = _goal_snapshot(
        _goal(),
        period_end="2026-08-01",
        activity={"planned_items": 1, "linked_items": 1},
    )
    no_due = _goal_snapshot(
        _goal(None),
        period_end="2026-08-01",
        activity={"planned_items": 1, "linked_items": 1},
    )
...
def test_deadline_risk_thresholds_are_stable():
    ...
    activity = {"planned_items": 1, "linked_items": 1}
    goal = _goal()
    goal["milestones"][0]["steps"].insert(
        0, {"id": "step-abcdef12", "status": "done", "due_date": "2026-07-10"}
    )
    values = []
    for remaining_days in (14, 7, 4, 3):
        day = f"2026-07-{31 - remaining_days:02d}"
        values.append(
            _goal_snapshot(goal, period_end=day, activity=activity)[
                "deadline_forecast"
            ]["status"]
        )
