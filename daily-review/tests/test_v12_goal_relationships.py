...
def test_parent_child_and_cycle_relationship_protection(tmp_path):
    ...
    self_parent = runner.invoke(
        app, ["goal", "edit", vision, "--parent", vision, "--root", str(tmp_path)]
    )
    cycle = runner.invoke(
        app, ["goal", "edit", vision, "--parent", medium, "--root", str(tmp_path)]
    )
    ...
    assert (
        "自分自身" in self_parent.output
        and "循環" in cycle.output
        and "存在しません" in missing.output
    )


def test_invalid_level_relation_archived_parent_and_path_traversal_are_rejected(
    tmp_path,
):
    _init(tmp_path)
    short = _add(tmp_path, "今日の作業", "short")
    invalid_relation = runner.invoke(
        app,
        [
            "goal",
            "add",
    ...
    assert (
        runner.invoke(
            app, ["goal", "archive", archived, "--yes", "--root", str(tmp_path)]
        ).exit_code
        == 0
    )
    ...
    traversal = runner.invoke(
        app, ["goal", "show", "../outside", "--root", str(tmp_path)]
    )
    assert (
        invalid_relation.exit_code == bad_parent.exit_code == traversal.exit_code == 3
    )
    assert (
        "level関係" in invalid_relation.output
        and "archived" in bad_parent.output
        and "ID" in traversal.output
    )
