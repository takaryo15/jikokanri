...
def test_reflect_text_creates_inbox_and_draft_and_can_stop_unapproved(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(cli, "_stdin_is_piped", lambda: False)
    result = runner.invoke(
        app, _reflect_args(tmp_path, "--text", SAFE_TEXT), input="n\n"
    )
...
def test_reflect_yes_approves_and_stdin_input_preserves_japanese(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0, result.output
    assert (
        load_daily(tmp_path, DAY)["tomorrow_plan_proposal"]["status"]
        == "pending_review"
    )
    ...
    stdin = runner.invoke(
        app,
        _reflect_args(stdin_root),
        input="研究はO VII/O VIIIを確認した。\n明日は院試を進める。\n",
    )
    assert stdin.exit_code == 0, stdin.output
    inbox = json.loads(
        (stdin_root / "data" / "inbox" / f"{DAY}.json").read_text(encoding="utf-8")
    )
    assert (
        inbox["entries"][0]["raw_text"]
...
def test_reflect_y_and_edit_then_return_to_confirmation(tmp_path, monkeypatch):
    ...
    approved = runner.invoke(
        app, _reflect_args(tmp_path, "--text", SAFE_TEXT), input="y\n"
    )
    ...
    edited = runner.invoke(
        app, _reflect_args(edit_root, "--text", SAFE_TEXT), input="e\nn\n"
    )
...
def test_reflect_resume_missing_and_approved_states(tmp_path, monkeypatch):
    ...
    assert (
        runner.invoke(
            app, _reflect_args(tmp_path, "--text", SAFE_TEXT, "--yes")
        ).exit_code
        == 0
    )
...
def test_reflect_dry_run_and_json_do_not_mix_output_or_write(tmp_path):
    ...
    as_json = runner.invoke(
        app, _reflect_args(tmp_path, "--text", SAFE_TEXT, "--dry-run", "--json")
    )
    ...
    saved_json = runner.invoke(
        app, _reflect_args(tmp_path / "saved", "--text", SAFE_TEXT, "--json")
    )
...
def test_reflect_rejects_empty_multiple_sources_and_auto_approval_risks(
    tmp_path, monkeypatch
):
    ...
    monkeypatch.setattr(cli, "_stdin_is_piped", lambda: False)
    unclassified = runner.invoke(
        app,
        _reflect_args(
            tmp_path / "unclassified", "--text", "なんとなく過ごした。", "--yes"
        ),
    )
    no_today = runner.invoke(
        app,
        _reflect_args(tmp_path / "no-today", "--text", "明日は研究を進める。", "--yes"),
    )
    no_tomorrow = runner.invoke(
        app,
        _reflect_args(
            tmp_path / "no-tomorrow", "--text", "院試の問題を解いた。", "--yes"
        ),
    )
...
def test_reflect_keeps_input_when_organization_fails_and_never_overwrites_daily(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(cli, "_stdin_is_piped", lambda: False)
    ...
    blocked = runner.invoke(
        app, _reflect_args(daily_root, "--text", SAFE_TEXT, "--yes")
    )
...
def test_reflect_duplicate_and_home_next_commands(tmp_path, monkeypatch):
    ...
    assert (
        runner.invoke(
            app, ["input", "--date", DAY, "--text", SAFE_TEXT, "--root", str(tmp_path)]
        ).exit_code
        == 0
    )
    ...
    empty_home = runner.invoke(
        app, ["home", "--date", DAY, "--root", str(tmp_path / "empty")]
    )
    assert f"daily-review handoff --date {DAY} --copy" in empty_home.output
