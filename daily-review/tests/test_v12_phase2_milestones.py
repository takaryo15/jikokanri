...
def test_milestone_add_list_show_json_edit_status_and_reorder(tmp_path):
    ...
    listed = runner.invoke(
        app, ["goal", "milestone", "list", goal_id, "--root", str(tmp_path)]
    )
    shown = runner.invoke(
        app,
        [
    ...
    assert all(
        result.exit_code == 0
        for result in (listed, shown, edited, completed, reordered)
    )
    ...
    assert next(item for item in stored["milestones"] if item["id"] == first)[
        "completed_at"
    ]
