...
def _proposal_file(tmp_path, **overrides):
    payload = {
        ...
        "main": ["院試", "研究", "健康"],
        "tasks": [
            {"area": "院試", "task": "過去問 大問1つ", "priority": 1, "minimum_line": "問題文だけ読む"}
        ],
...
def test_save_review_saves_structured_review_and_diary(tmp_path):
    review = {
        "diary": "任意の日記",
        "structured_review": {
            "today_main": [{"area": "院試", "status": "一部進んだ", "note": "過去問を少し見た"}],
            "minimum_line": {"院試": "達成"},
    ...
    result = runner.invoke(app, ["save-review", "--date", "2026-07-13", "--file", str(path), "--root", str(tmp_path)])
...
def test_proposal_and_final_are_separate_and_proposal_only_does_not_create_final(tmp_path):
    result = runner.invoke(
        app,
        ["save-proposal", "--date", "2026-07-13", "--file", str(_proposal_file(tmp_path)), "--root", str(tmp_path)],
    )
    assert result.exit_code == 0
    entry = load_daily(tmp_path, "2026-07-13")
...
def test_approve_plan_copies_proposal_to_final_with_approved_at(tmp_path):
    runner.invoke(app, ["save-proposal", "--date", "2026-07-13", "--file", str(_proposal_file(tmp_path)), "--root", str(tmp_path)])
    result = runner.invoke(app, ["approve-plan", "--date", "2026-07-13", "--root", str(tmp_path)])
    assert result.exit_code == 0
    entry = load_daily(tmp_path, "2026-07-13")
    assert entry["tomorrow_plan_proposal"]["status"] == "pending_review"
