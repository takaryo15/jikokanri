...
def test_daily_review_root_environment_is_used_after_explicit_root(
...
def test_migrate_creates_only_missing_v11_support_files_and_keeps_daily_data(tmp_path):
    ...
    dry = runner.invoke(
        app, ["migrate", "--root", str(tmp_path), "--dry-run", "--json"]
    )
    ...
    assert (
        ids.count(MIGRATION_ID)
        == ids.count(GOALS_MIGRATION_ID)
        == ids.count(ROADMAP_MIGRATION_ID)
        == ids.count(PLANNING_MIGRATION_ID)
        == ids.count(EVALUATION_MIGRATION_ID)
        == 1
    )
...
def test_v11_check_reports_json_and_detects_invalid_priorities(tmp_path):
    assert (
        runner.invoke(app, ["migrate", "--root", str(tmp_path), "--yes"]).exit_code == 0
    )
    passed = runner.invoke(app, ["v11-check", "--root", str(tmp_path), "--json"])
    assert passed.exit_code == 0, passed.output
    assert not json.loads(passed.output)["errors"]

    (tmp_path / "config" / "priorities.json").write_text(
        '{"priorities": []}', encoding="utf-8"
    )
