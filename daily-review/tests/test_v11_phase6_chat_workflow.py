...
def _source_file(root, payload=None):
    path = root / "chat.json"
    path.write_text(json.dumps(payload or _payload(), ensure_ascii=False), encoding="utf-8")
    return path
...
def test_chat_prompt_only_is_read_only_and_includes_context(tmp_path):
    ...
    previous = {
        ...
        "tomorrow_plan_final": {
            "target_date": DAY,
            "main": ["院試の過去問を2問解く"],
            "tasks": [{"id": "task-1", "area": "院試", "task": "院試の過去問を2問解く", "minimum_line": "問題を1問開く"}],
        },
        "task_results": [],
    }
    _write_json(tmp_path / "data" / "daily" / "2026-07-13.json", previous)
    _write_json(tmp_path / "data" / "daily" / "2026-07-15.json", {
        "date": "2026-07-15",
        "tomorrow_plan_proposal": {"tasks": [{"minimum_line": "翌日の最低ライン"}]},
    })
    result = runner.invoke(app, ["chat", "--date", DAY, "--prompt-only", "--root", str(tmp_path)])
...
def test_chat_prompt_copy_and_copy_failure_keep_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli.subprocess, "run", lambda *args, **kwargs: copied.setdefault("prompt", kwargs["input"]) or SimpleNamespace(stdout=""))
    copied_result = runner.invoke(app, ["chat", "--date", DAY, "--prompt-only", "--copy-prompt", "--root", str(tmp_path)])
    assert copied_result.exit_code == 0
    assert copied["prompt"]
    assert "コピーしました" in copied_result.output

    monkeypatch.setattr(cli.subprocess, "run", lambda *args, **kwargs: (_ for _ in ()).throw(OSError("clipboard")))
    failed = runner.invoke(app, ["chat", "--date", DAY, "--prompt-only", "--copy-prompt", "--root", str(tmp_path)])
    assert failed.exit_code == 0
    assert "WARN: コピーできなかった" in failed.output
    assert "schema_version" in failed.output


def test_chat_import_only_file_json_text_and_stdin_create_drafts_and_sessions(tmp_path, monkeypatch):
    for name, args in (
        ("file", ["--file", str(_source_file(tmp_path, _payload()))]),
        ("text", ["--json-text", json.dumps(_payload("2026-07-15"), ensure_ascii=False)]),
    ):
        root = tmp_path / name
        _init(root)
        if name == "file":
        ...
        result = runner.invoke(app, ["chat", "--import-only", "--date", "2026-07-14" if name == "file" else "2026-07-15", *args, "--root", str(root)])
        assert result.exit_code == 0, result.output
        assert (root / "data" / "drafts" / f"{'2026-07-14' if name == 'file' else '2026-07-15'}.json").is_file()
        session = json.loads(next((root / "data" / "sessions").glob("*.json")).read_text(encoding="utf-8"))
...
def test_chat_interactive_paste_retains_draft_and_resume_can_continue(tmp_path, monkeypatch):
    _init(tmp_path)
    monkeypatch.setattr(cli, "_stdin_is_piped", lambda: False)
    entered = json.dumps(_payload(), ensure_ascii=False, indent=2)
    result = runner.invoke(app, ["chat", "--date", DAY, "--root", str(tmp_path)], input=f"n\np\n{entered}\n__END__\nn\n")
    assert result.exit_code == 0, result.output
    assert "未承認ドラフトとして保存しました" in result.output
    resume = runner.invoke(app, ["chat", "--date", DAY, "--resume", "--root", str(tmp_path)], input="n\n")
    assert resume.exit_code == 0, resume.output
    assert "この内容を確定しますか？" in resume.output


def test_chat_detects_unapproved_draft_and_new_choice_never_overwrites_without_import(tmp_path, monkeypatch):
    _init(tmp_path)
    source = _source_file(tmp_path)
    assert runner.invoke(app, ["chat", "--date", DAY, "--import-only", "--file", str(source), "--root", str(tmp_path)]).exit_code == 0
    original = (tmp_path / "data" / "drafts" / f"{DAY}.json").read_text(encoding="utf-8")
    monkeypatch.setattr(cli, "_stdin_is_piped", lambda: False)
    result = runner.invoke(app, ["chat", "--date", DAY, "--root", str(tmp_path)], input="n\nn\nq\n")
    assert result.exit_code == 0, result.output
    assert "未承認のドラフトがあります" in result.output
    assert (tmp_path / "data" / "drafts" / f"{DAY}.json").read_text(encoding="utf-8") == original


def test_chat_yes_approval_completion_and_daily_only_protection(tmp_path):
    _init(tmp_path)
    source = _source_file(tmp_path)
    approved = runner.invoke(app, ["chat", "--date", DAY, "--import-only", "--file", str(source), "--yes", "--root", str(tmp_path)])
...
def test_chat_corrupt_session_does_not_modify_daily_data(tmp_path):
    ...
    source = _source_file(tmp_path)
    result = runner.invoke(app, ["chat", "--date", DAY, "--import-only", "--file", str(source), "--root", str(tmp_path)])
    assert result.exit_code == 0, result.output
