...
def _previous_night_payload():
    return {
        ...
        "diary": "前日の日記",
        "structured_review": {
            "today_main": [{"area": "院試", "status": "一部進んだ", "note": "問題文を見た"}],
            "minimum_line": {"院試": "達成"},
...
def _close_day_payload(**overrides):
    payload = {
        ...
        "task_results": [
            {"task_id": "task-1", "status": "completed", "note": "過去問の大問1を最後まで解いた", "minimum_line_achieved": True},
        ...
        "structured_review": {
            "today_main": [
                ...
                {"area": "筋トレ・健康", "status": "未着手", "note": "今日は実施しなかった"},
            ],
            "minimum_line": {"院試": "達成", "研究": "達成", "筋トレ・健康": "未達"},
            "what_went_well": ["院試の過去問を最後まで解けた", "研究も完全な未着手にはならなかった"],
        ...
        "tomorrow_plan_proposal": {
            ...
            "tasks": [
                ...
                {"area": "研究", "task": "RGS2のスペクトルを確認する", "priority": 2, "minimum_line": "RGS2の図を1枚開く"},
                {"area": "筋トレ・健康", "task": "胸トレを行う", "priority": 3, "minimum_line": "プロテインを飲む"},
            ],
            "one_change_tomorrow": "帰宅前に過去問を開く",
        },
...
def _close_day(tmp_path, payload=None, *extra_args):
    ...
    return runner.invoke(app, ["close-day", "--file", str(path), "--root", str(tmp_path), *extra_args])
...
def test_close_day_stores_task_results_in_previous_source_json(tmp_path):
    ...
    assert "task_results" not in load_daily(tmp_path, "2026-07-14") or not load_daily(tmp_path, "2026-07-14")["task_results"]
...
def test_close_day_preserves_existing_final_on_current_day(tmp_path):
    ...
    existing_final = {
        ...
        "target_date": "2026-07-15",
        "main": ["院試"],
        "tasks": [{"id": "task-1", "area": "院試", "task": "既存", "priority": 1, "minimum_line": "読む"}],
...
def test_close_day_keeps_unspecified_existing_results(tmp_path):
    _setup_previous_final(tmp_path)
    one_result = [{"task_id": "task-1", "status": "completed", "note": "先に保存", "minimum_line_achieved": True}]
...
def test_close_day_unknown_task_id_changes_neither_file(tmp_path):
    _setup_previous_final(tmp_path)
    payload = _close_day_payload(task_results=[{"task_id": "missing", "status": "completed", "note": "", "minimum_line_achieved": True}])
    before_previous = (tmp_path / "data" / "daily" / "2026-07-13.json").read_text(encoding="utf-8")
    result = _close_day(tmp_path, payload)
    assert result.exit_code != 0
    assert (tmp_path / "data" / "daily" / "2026-07-13.json").read_text(encoding="utf-8") == before_previous
    assert load_daily(tmp_path, "2026-07-14") is None


def test_close_day_invalid_status_changes_neither_file(tmp_path):
    _setup_previous_final(tmp_path)
    payload = _close_day_payload(task_results=[{"task_id": "task-1", "status": "done", "note": "", "minimum_line_achieved": True}])
    result = _close_day(tmp_path, payload)
    assert result.exit_code != 0
    assert load_daily(tmp_path, "2026-07-14") is None
...
def test_close_day_dry_run_does_not_change_files(tmp_path):
    ...
    assert result.exit_code == 0
    assert (tmp_path / "data" / "daily" / "2026-07-13.json").read_text(encoding="utf-8") == before
    assert load_daily(tmp_path, "2026-07-14") is None
...
def test_close_day_updates_both_markdown_logs(tmp_path):
    _setup_previous_final(tmp_path)
    assert _close_day(tmp_path).exit_code == 0
    previous_markdown = (tmp_path / "logs" / "2026-07-13.md").read_text(encoding="utf-8")
...
def test_close_day_temp_write_failure_keeps_original_files(tmp_path, monkeypatch):
    ...
    result = _close_day(tmp_path)
    assert result.exit_code != 0
    assert (current_dir / "2026-07-13.json").read_text(encoding="utf-8") == before_previous
    assert (current_dir / "2026-07-14.json").read_text(encoding="utf-8") == before_current
