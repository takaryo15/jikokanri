...
def test_input_saves_text_multiline_japanese_and_preserves_source(tmp_path):
    ...
    result = runner.invoke(app, ["input", "--date", "2026-07-14", "--text", raw, "--root", str(tmp_path)])
...
def test_input_appends_without_destroying_existing_entries_or_fields(tmp_path):
    first = runner.invoke(app, ["input", "--date", "2026-07-14", "--text", "最初", "--root", str(tmp_path)])
    second = runner.invoke(app, ["input", "--date", "2026-07-14", "--text", "次", "--root", str(tmp_path)])
...
def test_input_dry_run_and_empty_input_do_not_write(tmp_path):
    dry_run = runner.invoke(app, ["input", "--date", "2026-07-14", "--text", "確認", "--dry-run", "--root", str(tmp_path)])
    empty = runner.invoke(app, ["input", "--date", "2026-07-14", "--text", "   ", "--root", str(tmp_path)])
...
def test_input_rejects_multiple_sources_and_reads_stdin(tmp_path):
    multiple = runner.invoke(app, ["input", "--text", "a", "--clipboard", "--root", str(tmp_path)])
    stdin = runner.invoke(app, ["input", "--date", "2026-07-14", "--root", str(tmp_path)], input="標準入力の原文\n")
...
def test_input_reads_clipboard_without_modifying_raw_text(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli.subprocess, "run", lambda *args, **kwargs: SimpleNamespace(stdout="クリップボード\n原文"))
    result = runner.invoke(app, ["input", "--date", "2026-07-14", "--clipboard", "--root", str(tmp_path)])
    assert result.exit_code == 0
...
def test_input_reports_corrupt_existing_inbox_without_overwriting(tmp_path):
    ...
    result = runner.invoke(app, ["input", "--date", "2026-07-14", "--text", "新規", "--root", str(tmp_path)])
