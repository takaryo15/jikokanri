...
def _add_goal(root, title):
    result = runner.invoke(
        app, ["goal", "add", "--title", title, "--level", "medium", "--root", str(root)]
    )
...
def _add_mile(root, goal_id, title):
    result = runner.invoke(
        app,
        ["goal", "milestone", "add", goal_id, "--title", title, "--root", str(root)],
    )
    assert result.exit_code == 0, result.output
    return result.output.split("ID: ", 1)[1].splitlines()[0]
...
def test_milestone_dependency_rejects_self_missing_and_cycle(tmp_path):
    ...
    first, second = (
        _add_mile(tmp_path, goal_id, "一周目"),
        _add_mile(tmp_path, goal_id, "二周目"),
    )
    self_ref = runner.invoke(
        app,
        [
            "goal",
            "milestone",
    ...
    assert (
        "自分自身" in self_ref.output
        and "存在しません" in missing.output
        and "循環" in cycle.output
    )


def test_step_dependency_rejects_cross_milestone_and_completed_reverse_dependency(
    tmp_path,
):
    assert runner.invoke(app, ["init", "--root", str(tmp_path)]).exit_code == 0
    goal_id = _add_goal(tmp_path, "院試")
