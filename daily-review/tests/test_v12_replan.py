...
def test_replan_requires_selected_proposal_and_applies_transactionally(tmp_path):
    ...
    replan_id = value["id"]
    proposal_id = value["proposals"][0]["id"]
    before = (tmp_path / "data" / "goals" / "items" / f"{goal_id}.json").read_bytes()
    rejected = runner.invoke(
        app, ["goal", "replan", "apply", replan_id, "--yes", "--root", str(tmp_path)]
    )
    assert (
        rejected.exit_code == 3
        and (tmp_path / "data" / "goals" / "items" / f"{goal_id}.json").read_bytes()
        == before
    )
    ...
    goal = json.loads(
        (tmp_path / "data" / "goals" / "items" / f"{goal_id}.json").read_text(
            encoding="utf-8"
        )
    )
    assert goal["status"] == "paused"
    assert (
        goal["history"][-1]["source"] == "replan"
        and goal["history"][-1]["replan_id"] == replan_id
    )
    assert (
        json.loads(
            (tmp_path / "data" / "replans" / f"{replan_id}.json").read_text(
                encoding="utf-8"
            )
        )["status"]
        == "applied"
...
def test_replan_rejects_unknown_edit_field_and_can_cancel(tmp_path):
    ...
    rejected = runner.invoke(
        ...
        [
            ...
            replan_id,
            "--set",
            f"{proposal_id}.after.unknown=x",
            "--root",
            str(tmp_path),
        ],
    )
    cancelled = runner.invoke(
        app, ["goal", "replan", "cancel", replan_id, "--yes", "--root", str(tmp_path)]
    )
...
def test_replan_backup_failure_leaves_goal_and_replan_unchanged(tmp_path, monkeypatch):
    ...
    assert (
        goal_path.read_bytes() == before_goal
        and replan_path.read_bytes() == before_replan
    )
...
def test_reduce_daily_load_updates_separate_planning_config(tmp_path):
    _setup(tmp_path)
    evaluation_path = (
        tmp_path / "data" / "evaluations" / "weekly" / "2026-07-14_2026-07-20.json"
    )
    ...
    assert (
        json.loads((tmp_path / "config" / "planning.json").read_text(encoding="utf-8"))[
            "max_daily_main"
        ]
        == 2
    )
