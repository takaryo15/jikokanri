...
def test_goal_plan_result_evaluation_replan_rc1_flow(tmp_path):
    ...
    replan_payload = json.loads(replan.output)
    replan_id = replan_payload["id"]
    proposal_id = replan_payload["proposals"][0]["id"]
    assert (
        runner.invoke(
            app,
            [
    ...
    assert (
        runner.invoke(
            app,
            ["goal", "replan", "apply", replan_id, "--yes", "--root", str(tmp_path)],
        ).exit_code
        == 0
    )
