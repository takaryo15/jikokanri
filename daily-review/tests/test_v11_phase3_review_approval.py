...
def test_review_renders_draft_json_and_missing_draft_error(tmp_path):
    ...
    review = runner.invoke(
        app, ["review", "--date", DAY, "--dry-run", "--root", str(tmp_path)]
    )
    as_json = runner.invoke(
        app, ["review", "--date", DAY, "--json", "--root", str(tmp_path)]
    )
...
def test_approve_yes_saves_compatible_daily_content_and_marks_draft(tmp_path):
    ...
    assert (
        daily["tomorrow_plan_final"] if "tomorrow_plan_final" in daily else None is None
    )
...
def test_approve_cancel_and_noninteractive_mode_do_not_save(tmp_path, monkeypatch):
    ...
    cancelled = runner.invoke(
        app, ["approve", "--date", DAY, "--root", str(tmp_path)], input="n\n"
    )
...
def test_reapproval_requires_force_and_creates_daily_backup(tmp_path):
    ...
    forced = runner.invoke(
        app, ["approve", "--date", DAY, "--force", "--yes", "--root", str(tmp_path)]
    )
...
def test_edit_draft_set_replaces_lists_deduplicates_and_tracks_history(tmp_path):
    ...
    result = runner.invoke(
        ...
        [
            ...
            DAY,
            "--set",
            "tomorrow.main_candidates=院試を2問解く",
            "--set",
            "tomorrow.main_candidates=研究を進める",
            "--set",
            "today.completed=同じ内容",
            "--set",
            "today.completed=同じ内容",
            "--root",
            str(tmp_path),
    ...
    assert draft["edit_history"][-1]["changed_fields"] == [
...
def test_edit_draft_rejects_invalid_edits_and_force_edit_resets_approval(tmp_path):
    ...
    too_many = runner.invoke(
        ...
        [
            ...
            DAY,
            "--set",
            "today.main_candidates=a",
            "--set",
            "today.main_candidates=b",
            "--set",
            "today.main_candidates=c",
            "--set",
            "today.main_candidates=d",
            "--root",
            str(tmp_path),
        ],
    )
