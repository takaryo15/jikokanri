...
def _ids(root):
    ...
    return (
        goal_id,
        first.output.split("ID: ", 1)[1].splitlines()[0],
        second.output.split("ID: ", 1)[1].splitlines()[0],
    )
...
def test_roadmap_json_compact_next_and_home_actions(tmp_path):
    ...
    compact = runner.invoke(
        app, ["goal", "roadmap", goal_id, "--compact", "--root", str(tmp_path)]
    )
    raw = runner.invoke(
        app, ["goal", "roadmap", goal_id, "--json", "--root", str(tmp_path)]
    )
    ...
    assert all(
        result.exit_code == 0 for result in (roadmap, compact, raw, next_item, home)
    )
    assert json.loads(raw.output)["milestones"][0]["id"] == first
    assert "2025年度" in roadmap.output and "2025年度" not in compact.output
    assert (
        "2025年度" in next_item.output
    )  # doing is preferred over an earlier due todo step
