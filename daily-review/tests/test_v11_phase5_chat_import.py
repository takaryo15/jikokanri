...
def test_chat_import_accepts_one_fenced_json_and_preserves_multiline_japanese(tmp_path):
    ...
    result = runner.invoke(
        app, ["chat-import", "--json-text", content, "--root", str(tmp_path)]
    )
    assert result.exit_code == 0, result.output
...
def test_chat_import_dry_run_and_output_json_do_not_write(tmp_path):
    result = runner.invoke(
        app, _args(tmp_path, _payload(), "--dry-run", "--output-json")
    )
...
def test_chat_import_rejects_invalid_json_version_required_types_and_values(tmp_path):
    ...
    bad_json = runner.invoke(
        app, ["chat-import", "--json-text", "{", "--root", str(tmp_path)]
    )
    ...
    assert (
        ...
        == missing_result.exit_code
        == wrong_type.exit_code
        == blank.exit_code
        == duplicate.exit_code
        == 3
    )
...
def test_chat_import_rejects_date_mismatch_large_input_and_duplicate_import(tmp_path):
    ...
    huge = runner.invoke(
        app, _args(tmp_path / "huge", _payload(raw_text="あ" * 100_001))
    )
...
def test_chat_import_yes_rejects_unclassified_and_approved_draft_cannot_be_replaced(
    tmp_path,
):
    unclassified = runner.invoke(
        app,
        _args(tmp_path / "unclassified", _payload(unclassified=["判断不能"]), "--yes"),
    )
    ...
    rejected = runner.invoke(
        app, _args(approved_root, _payload(raw_text="別の原文"), "--force")
    )
...
def test_chat_import_stdin_file_and_corrupt_inbox_are_safe(tmp_path, monkeypatch):
    ...
    source.write_text(
        json.dumps(_payload(raw_text="別日内容"), ensure_ascii=False), encoding="utf-8"
    )
    ...
    corrupt = runner.invoke(
        app, ["chat-import", "--file", str(source), "--root", str(corrupt_root)]
    )
...
def test_chat_prompt_date_and_clipboard_and_file_dry_run(tmp_path, monkeypatch):
    ...
    copied_result = runner.invoke(
        app, ["chat-prompt", "--clipboard", "--root", str(tmp_path)]
    )
    source = tmp_path / "reflection.json"
    source.write_text(json.dumps(_payload(), ensure_ascii=False), encoding="utf-8")
    file_dry = runner.invoke(
        app,
        ["chat-import", "--file", str(source), "--dry-run", "--root", str(tmp_path)],
    )
