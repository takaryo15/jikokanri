...
def test_add_all_levels_list_show_and_json(tmp_path):
    _init(tmp_path)
    ids = [
        _add(tmp_path, level=level) for level in ("vision", "long", "medium", "short")
    ]
    listing = runner.invoke(
        app, ["goal", "list", "--level", "medium", "--root", str(tmp_path)]
    )
    shown = runner.invoke(app, ["goal", "show", ids[2], "--root", str(tmp_path)])
    rendered = runner.invoke(
        app, ["goal", "show", ids[2], "--json", "--root", str(tmp_path)]
    )
...
def test_edit_status_archive_and_backup(tmp_path):
    _init(tmp_path)
    goal_id = _add(
        tmp_path, extra=("--start-date", "2026-07-14", "--due-date", "2026-08-31")
    )
    ...
    completed = runner.invoke(
        app, ["goal", "status", goal_id, "completed", "--root", str(tmp_path)]
    )
    stored = json.loads(
    ...
    resumed = runner.invoke(
        app, ["goal", "status", goal_id, "active", "--root", str(tmp_path)]
    )
    archived = runner.invoke(
        app, ["goal", "archive", goal_id, "--yes", "--root", str(tmp_path)]
    )
    ...
    assert (
        ...
        and stored["history"][-2]["changed_fields"] == ["title", "due_date"]
    )
    assert (
        json.loads(
            (tmp_path / "data" / "goals" / "items" / f"{goal_id}.json").read_text(
...
def test_goal_metrics_qualitative_home_doctor_and_release_check(tmp_path):
    ...
    assert (
        "OK   goals schema" in doctor.output and "OK   goal commands" in release.output
    )
