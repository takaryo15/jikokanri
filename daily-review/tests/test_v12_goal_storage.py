...
def test_progress_calculations_and_manual_fallback_are_clamped():
    increase = _goal(quantitative_metrics=[{"id": "metric-a", "name": "回数", "unit": "回", "baseline": 0, "target": 3, "current": 5, "direction": "increase"}])
    decrease = _goal(quantitative_metrics=[{"id": "metric-a", "name": "体脂肪", "unit": "%", "baseline": 30, "target": 20, "current": 25, "direction": "decrease"}])
    boolean = _goal(quantitative_metrics=[{"id": "metric-a", "name": "提出", "unit": "", "baseline": False, "target": True, "current": True, "direction": "boolean"}])
    qualitative = _goal(qualitative_criteria=[{"id": "qual-a", "description": "説明できる", "status": "partially_met"}])
...
def test_invalid_dates_and_backup_failure_do_not_modify_existing_goal(tmp_path, monkeypatch):
    assert runner.invoke(app, ["init", "--root", str(tmp_path)]).exit_code == 0
    invalid = runner.invoke(app, ["goal", "add", "--title", "bad", "--level", "short", "--start-date", "2026-07-15", "--due-date", "2026-07-14", "--root", str(tmp_path)])
    assert invalid.exit_code == 3
    created = runner.invoke(app, ["goal", "add", "--title", "before", "--level", "short", "--root", str(tmp_path)])
    goal_id = created.output.split("ID: ", 1)[1].splitlines()[0]
    path = tmp_path / "data" / "goals" / "items" / f"{goal_id}.json"
    before = path.read_text(encoding="utf-8")
    monkeypatch.setattr(goals.shutil, "copy2", lambda *_args: (_ for _ in ()).throw(OSError("backup failed")))
    failed = runner.invoke(app, ["goal", "edit", goal_id, "--title", "after", "--root", str(tmp_path)])
