...
def _save_entry(root, source_day: str, target_day: str) -> None:
    ...
    path.write_text(json.dumps({
        "date": source_day, "created_at": f"{source_day}T22:00:00+09:00", "updated_at": f"{source_day}T22:00:00+09:00",
        "structured_review": {"minimum_line": {"院試": "達成"}, "breakdown_causes": ["睡眠不足"], "what_went_well": ["早起きできた"]},
        "tomorrow_plan_final": {"status": "approved", "target_date": target_day, "main": ["院試"], "tasks": [{"id": "task-1", "area": "院試", "task": "過去問", "priority": 1, "minimum_line": "開く"}], "one_change_tomorrow": "続ける"},
        "task_results": [{"task_id": "task-1", "status": "partial", "minimum_line_achieved": True}],
...
def test_phase7_monthly_saves_both_formats_and_handles_leap_year(tmp_path):
    ...
    result = runner.invoke(app, ["monthly", "--date", "2024-02-14", "--root", str(tmp_path)])
