...
def _payload(**overrides):
    value = {
        ...
        "workflow": "goal_coach",
        "period_type": "week",
        "period_id": "2026-07-14_2026-07-20",
        "analysis": {
            "strengths": ["記録を続けた"],
            "problems": [],
            "root_causes": [],
            "patterns": [],
            "recommendations": ["Mainを減らす"],
            "evidence": ["予定過多2日"],
        },
    }
...
def test_coach_prompt_and_receive_store_only_auxiliary_analysis(tmp_path):
    ...
    prompt = runner.invoke(
        app, ["goal", "coach", "--week", "2026-07-14", "--root", str(tmp_path)]
    )
    ...
    assert (
        "医療・心理診断をしない" in prompt.output
        and "自動適用は行っていません" in received.output
    )
