...
def test_weekly_and_daily_goal_plans_are_drafts_then_explicitly_approved(tmp_path):
    ...
    preview = runner.invoke(
        app, ["plan", "week", "--date", "2026-07-14", "--json", "--root", str(tmp_path)]
    )
    saved = runner.invoke(
        app, ["plan", "week", "--date", "2026-07-14", "--save", "--root", str(tmp_path)]
    )
    applied = runner.invoke(
        app, ["plan", "apply", "--week", "2026-07-14", "--yes", "--root", str(tmp_path)]
    )
    daily = runner.invoke(
        app,
        ["plan", "today", "--date", "2026-07-14", "--save", "--root", str(tmp_path)],
    )
    daily_applied = runner.invoke(
        app, ["plan", "apply", "--date", "2026-07-14", "--yes", "--root", str(tmp_path)]
    )
    assert all(
        result.exit_code == 0
        for result in (preview, saved, applied, daily, daily_applied)
    )
    weekly = json.loads(preview.output)
    assert (
        weekly["week_start"] == "2026-07-14"
        and weekly["focus_items"][0]["step_id"] == step_id
    )
    stored = json.loads(
        (tmp_path / "data" / "plans" / "daily" / "2026-07-14.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_review_link_progress_and_doctor(tmp_path):
    ...
    assert (
        runner.invoke(
            app,
            ["plan", "week", "--date", "2026-07-14", "--save", "--root", str(tmp_path)],
        ).exit_code
        == 0
    )
    assert (
        runner.invoke(
            app,
            ["plan", "apply", "--week", "2026-07-14", "--yes", "--root", str(tmp_path)],
        ).exit_code
        == 0
    )
    assert (
...
def test_doctor_rejects_invalid_goal_plan_without_changing_daily_data(tmp_path):
    ...
    path.write_text(
        json.dumps(
            {"date": "2026-07-14", "status": "invalid", "main_candidates": []},
            ensure_ascii=False,
        ),
        encoding="utf-8",
    )
