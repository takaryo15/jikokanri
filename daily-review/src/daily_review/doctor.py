...
from .archive import list_backups
from .configuration import validate_config
from .migration import current_schema_version, load_migration_history
from .recovery import restore_history
...
def _check_daily(path: Path, issues: list[dict[str, str]]) -> None:
    try:
        ...
        issues.append(_issue("ERROR", f"日次JSONを読み込めません: {path.name} ({exc})"))
        return
    if not isinstance(entry, dict) or not isinstance(entry.get("date"), str):
        issues.append(_issue("ERROR", f"日次JSONの必須dateが不正です: {path.name}"))
        return
    proposal, final = (
        entry.get("tomorrow_plan_proposal"),
    ...
    if proposal:
        ...
        if proposal.get("status") == "approved":
            issues.append(
                _issue("ERROR", f"{path.name}: proposalがapprovedになっています")
            )
    if final:
    ...
    for item in entry.get("task_results") or []:
        if item.get("status") not in ALLOWED_TASK_RESULT_STATUSES:
            issues.append(
                _issue(
                    "ERROR",
                    f"{path.name}: 実行結果のstatusが不正です（{item.get('status')}）",
                )
            )
...
def run_doctor(root: Path) -> dict[str, Any]:
        ...
        if not path.is_dir():
            if relative == Path("data/inbox"):
                issues.append(
                    _issue(
                        "WARN",
                        "data/inbox がありません。daily-review input 実行時に自動作成されます",
                    )
                )
                ...
                issues.append(
                    _issue(
                        "WARN",
                        "data/drafts がありません。daily-review organize 実行時に自動作成されます",
                    )
                )
                ...
                issues.append(
                    _issue(
                        "WARN",
                        "data/sessions がありません。daily-review chat 実行時に自動作成されます",
                    )
                )
                ...
                issues.append(
                    _issue(
                        "WARN",
                        "data/handoffs がありません。daily-review handoff 実行時に自動作成されます",
                    )
                )
                ...
                issues.append(
                    _issue(
                        "WARN",
                        f"{relative} がありません。daily-review api 実行時に自動作成されます",
                    )
                )
                ...
                issues.append(
                    _issue(
                        "WARN",
                        f"{relative} がありません。対応コマンドの初回実行時に自動作成されます",
                    )
                )
            else:
                issues.append(
                    _issue("ERROR", f"必要なディレクトリがありません: {relative}")
                )
    ...
    for name in REQUIRED_TEMPLATE_NAMES:
        if not (root / "templates" / name).is_file():
            issues.append(
                _issue("ERROR", f"必要なテンプレートがありません: templates/{name}")
            )
    ...
    if chat_prompt.is_file():
        try:
            ...
            issues.append(
                _issue("ERROR", f"ChatGPT取込テンプレートを読み込めません: {exc}")
            )
        else:
            ...
            if all(field in prompt_text for field in required_prompt_fields):
                ...
                issues.append(
                    _issue(
                        "ERROR",
                        "chat import promptのschema_versionまたは必須変数が不足しています",
                    )
                )
    else:
        issues.append(
            _issue(
                "WARN",
                f"ChatGPT取込テンプレートがありません: templates/{CHAT_IMPORT_PROMPT_NAME}（daily-review init で作成できます）",
            )
        )
    if SCHEMA_VERSION == "1.0":
        ...
        issues.append(
            _issue(
                "ERROR", f"chat import schemaのバージョンが不正です: {SCHEMA_VERSION}"
            )
        )
    ...
    for path in daily_files:
        ...
        if not (root / "logs" / f"{path.stem}.md").is_file():
            issues.append(
                _issue("WARN", f"Markdownが存在しない日次データ: {path.name}")
            )
    for folder, prefix in (("weekly", "weekly_"), ("monthly", "monthly_")):
        ...
        for path in sorted(directory.glob("*.json")) if directory.is_dir() else []:
            try:
                ...
                issues.append(
                    _issue(
                        "ERROR", f"{folder}JSONを読み込めません: {path.name} ({exc})"
                    )
                )
                continue
            markdown_name = (
                f"{prefix}{path.stem}.md"
                if folder == "weekly"
                else f"monthly_{path.stem}.md"
            )
            if not (root / "logs" / markdown_name).is_file():
                issues.append(
                    _issue("WARN", f"Markdownが存在しない{folder}データ: {path.name}")
                )
    ...
    if inbox_dir.is_dir():
        for path in sorted(inbox_dir.glob("*.json")):
            try:
                ...
                issues.append(
                    _issue("ERROR", f"inbox JSONを読み込めません: {path.name} ({exc})")
                )
    ...
    if drafts_dir.is_dir():
        for path in sorted(drafts_dir.glob("*.json")):
            try:
                value = read_json_file(path)
                if not isinstance(value, dict) or not isinstance(
                    value.get("source_entry_ids"), list
                ):
                    raise ValueError("source_entry_idsがありません")
            except (OSError, ValueError) as exc:
                issues.append(
                    _issue("ERROR", f"draft JSONを読み込めません: {path.name} ({exc})")
                )
            ...
            if status is None:
                issues.append(
                    _issue(
                        "WARN",
                        f"{path.name}: statusがありません（旧ドラフトとしてdraft扱いです）",
                    )
                )
            elif status not in {"draft", "approved"}:
                issues.append(
                    _issue("ERROR", f"{path.name}: draft statusが不正です（{status}）")
                )
                draft_status_ok = False
            elif status == "approved":
                if (
                    not isinstance(value.get("approved_at"), str)
                    or not value["approved_at"].strip()
                ):
                    issues.append(
                        _issue(
                            "ERROR",
                            f"{path.name}: approvedなのにapproved_atがありません",
                        )
                    )
                    draft_status_ok = False
                approved_path = value.get("approved_daily_path")
                target = (
                    root / approved_path if isinstance(approved_path, str) else None
                )
                if not approved_path or not target or not target.is_file():
                    issues.append(
                        _issue(
                            "ERROR",
                            f"{path.name}: approvedなのに確定日次ファイルがありません",
                        )
                    )
                    draft_status_ok = False
            elif value.get("approved_at") or value.get("approved_daily_path"):
                issues.append(
                    _issue("WARN", f"{path.name}: draftなのに承認情報が残っています")
                )
            for field in ("today.main_candidates", "tomorrow.main_candidates"):
                ...
                if isinstance(values, list) and len(values) > 3:
                    issues.append(
                        _issue("ERROR", f"{path.name}: {field} は最大3件です")
                    )
                    draft_status_ok = False
    ...
    if sessions_dir.is_dir():
        for path in sorted(sessions_dir.glob("*.json")):
            try:
                ...
                issues.append(
                    _issue("WARN", f"chat sessionを読み込めません: {path.name} ({exc})")
                )
                sessions_ok = False
                continue
            if (
                status == "approved"
                and not (root / "data" / "daily" / f"{value['date']}.json").is_file()
            ):
                issues.append(
                    _issue("WARN", f"{path.name}: approvedなのに日次データがありません")
                )
                sessions_ok = False
            if (
                status == "draft"
                and not (root / "data" / "drafts" / f"{value['date']}.json").is_file()
            ):
                issues.append(
                    _issue("WARN", f"{path.name}: draftなのにドラフトがありません")
                )
                sessions_ok = False
    if sessions_ok:
        ...
        for path in sorted(handoffs_dir.glob("*.json")):
            try:
                ...
                if (
                    ...
                    or not isinstance(value.get("handoffs"), list)
                ):
                    raise ValueError("dateまたはhandoffsが不正です")
            except (OSError, ValueError) as exc:
                issues.append(
                    _issue(
                        "ERROR", f"handoff JSONを読み込めません: {path.name} ({exc})"
                    )
                )
            ...
            for item in value["handoffs"]:
                if not isinstance(item, dict):
                    issues.append(
                        _issue("ERROR", f"{path.name}: handoff項目が不正です")
                    )
                    handoffs_ok = False
                    continue
                ...
                if (
                    ...
                    issues.append(
                        _issue(
                            "ERROR",
                            f"{path.name}: session_idが不正または重複しています",
                        )
                    )
                    handoffs_ok = False
                else:
                ...
                if status not in HANDOFF_STATUSES:
                    issues.append(
                        _issue("ERROR", f"{path.name}: handoff statusが不正です")
                    )
                    handoffs_ok = False
                if not isinstance(item.get("prompt_hash"), str) or not item[
                    "prompt_hash"
                ].startswith("sha256:"):
                    issues.append(
                        _issue("ERROR", f"{path.name}: prompt_hashがありません")
                    )
                    handoffs_ok = False
                ...
                if isinstance(content_hash, str):
                    if content_hash in seen_hashes:
                        issues.append(
                            _issue("ERROR", f"{path.name}: import_hashが重複しています")
                        )
                        handoffs_ok = False
                ...
                if (
                    status == "approved"
                    and not (
                        root / "data" / "daily" / f"{value['date']}.json"
                    ).is_file()
                ):
                    issues.append(
                        _issue(
                            "ERROR",
                            f"{path.name}: approvedなのに日次データがありません",
                        )
                    )
                    handoffs_ok = False
                if (
                    status == "received"
                    and not (
                        root / "data" / "drafts" / f"{value['date']}.json"
                    ).is_file()
                ):
                    issues.append(
                        _issue(
                            "WARN", f"{path.name}: receivedなのにドラフトがありません"
                        )
                    )
                ...
                if status == "issued":
                    try:
                        if is_expired(item):
                            issues.append(
                                _issue(
                                    "WARN", f"{path.name}: issued handoffが期限切れです"
                                )
                            )
                            handoff_consistent = False
                    except HandoffError as exc:
                        issues.append(_issue("ERROR", f"{path.name}: {exc}"))
                        handoffs_ok = False
                    ...
                    try:
                        ...
                        if (
                            ...
                            issues.append(
                                _issue(
                                    "WARN",
                                    f"{path.name}: handoffとsessionのstatusが不整合です",
                                )
                            )
                            handoff_consistent = False
                        ...
                        issues.append(
                            _issue(
                                "WARN",
                                f"{path.name}: 対応するchat sessionを読み込めません",
                            )
                        )
                        handoff_consistent = False
    if handoffs_ok:
    ...
    if HANDOFF_VERSION == "1.0":
        ...
        issues.append(
            _issue("ERROR", f"handoff schemaのバージョンが不正です: {HANDOFF_VERSION}")
        )
    ...
    if not priority_file.is_file():
        issues.append(
            _issue(
                "WARN",
                "優先順位設定がありません: config/priorities.json（daily-review init で作成できます）",
            )
        )
    else:
        try:
            ...
            for milestone in milestones_of(goal):
                ...
                if milestone.get("status") == "completed" and pending:
                    issues.append(
                        _issue(
                            "WARN",
                            f"goal {goal['id']}: completedマイルストーンに未完了ステップがあります",
                        )
                    )
    ...
    for path in (
        sorted((root / "data" / "plans" / "weekly").glob("*.json"))
        if (root / "data" / "plans" / "weekly").is_dir()
        else []
    ):
        try:
            value = read_json_file(path)
            validate_weekly_plan(value)
            for item in (value.get("focus_items") or []) + (
                value.get("other_candidates") or []
            ):
                _validate_ref(root, item)
        except (OSError, ValueError, PlanningError, GoalError) as exc:
            issues.append(
                _issue("ERROR", f"週次目標計画を読み込めません: {path.name} ({exc})")
            )
            weekly_plans_ok = links_ok = False
    for path in (
        sorted((root / "data" / "plans" / "daily").glob("*.json"))
        if (root / "data" / "plans" / "daily").is_dir()
        else []
    ):
        try:
            value = read_json_file(path)
            validate_daily_plan(value)
            source = {
                "main": value.get("main_candidates") or [],
                "task": value.get("other_tasks") or [],
            }
            for link in value.get("goal_links") or []:
                items = source.get(link["record_type"], [])
                if not 1 <= link["record_index"] <= len(items):
                    raise PlanningError("goal linkの参照先がありません")
                _validate_ref(root, link)
        except (OSError, ValueError, PlanningError, GoalError, KeyError) as exc:
            issues.append(
                _issue("ERROR", f"日次目標計画を読み込めません: {path.name} ({exc})")
            )
            daily_plans_ok = links_ok = progress_ok = False
    if weekly_plans_ok:
        checks.append("weekly goal plans")
    if daily_plans_ok:
        checks.append("daily goal plans")
    if links_ok:
        checks.append("goal links")
    if progress_ok:
        checks.append("goal progress consistency")
    ...
    if planning_config.exists():
        try:
            ...
            if (
                ...
                or not isinstance(config.get("max_daily_main"), int)
                or isinstance(config.get("max_daily_main"), bool)
                or not 1 <= config["max_daily_main"] <= 3
            ):
    ...
    for period_type, directory in (
        ("week", root / "data" / "evaluations" / "weekly"),
        ("month", root / "data" / "evaluations" / "monthly"),
    ):
        for path in sorted(directory.glob("*.json")) if directory.is_dir() else []:
            try:
                value = read_json_file(path)
                ...
                if any(
                    item.get("goal_id") not in known_goal_ids
                    for item in value.get("goal_evaluations") or []
                ):
                    raise EvaluationError("存在しないgoalの評価があります")
                if not all(
                    ...
                    raise EvaluationError("diagnosticsが不正です")
                coach = value.get("coach_analysis")
                if coach is not None and (
                    not isinstance(coach, dict)
                    or not isinstance(coach.get("evidence", []), list)
                ):
                    raise EvaluationError("coach分析が不正です")
            except (OSError, ValueError, EvaluationError) as exc:
                issues.append(
                    _issue("ERROR", f"goal評価を読み込めません: {path.name} ({exc})")
                )
                evaluations_ok = diagnostics_ok = coach_ok = False
    if evaluations_ok:
        checks.append("goal evaluations")
    if diagnostics_ok:
        checks.append("goal diagnostics")
    if coach_ok:
        checks.append("goal coach data")
    ...
    for path in sorted(replan_dir.glob("replan-*.json")) if replan_dir.is_dir() else []:
        try:
            ...
            known_goal_ids = {goal.get("id") for goal in goals}
            if any(
                item.get("goal_id") is not None
                and item.get("goal_id") not in known_goal_ids
                for item in value.get("proposals") or []
            ):
                raise ReplanError("存在しないgoalへのproposalがあります")
            if value.get("status") == "applied" and not value.get(
                "approved_proposal_ids"
            ):
                raise ReplanError("適用済みreplanに承認proposalがありません")
        except (OSError, ValueError, ReplanError) as exc:
            issues.append(
                _issue("ERROR", f"replanを読み込めません: {path.name} ({exc})")
            )
            replans_ok = application_ok = False
    if replans_ok:
        checks.append("goal replans")
    if application_ok:
        checks.append("replan application consistency")
    ...
    for path in sorted(design_dir.glob("design-*.json")) if design_dir.is_dir() else []:
        try:
            ...
            if value.get("status") == "applied" and value.get(
                "applied_goal_id"
            ) not in {goal.get("id") for goal in goals}:
                raise GoalDesignError("appliedなのに対応するgoalがありません")
        except (OSError, ValueError, GoalDesignError) as exc:
            issues.append(
                _issue(
                    "ERROR", f"目標設計セッションを読み込めません: {path.name} ({exc})"
                )
            )
            design_ok = False
    if design_ok:
    ...
    for path in (
        sorted(transaction_dir.glob("transaction-*.json"))
        ...
        try:
            ...
            if not isinstance(value, dict) or value.get("status") not in {
                ...
                raise ValueError(
                    "未完了transactionです。元データを確認後、同じreplan applyを再実行してください"
                )
        except (OSError, ValueError) as exc:
    ...
    if missing_history:
        issues.append(
            _issue(
                "ERROR",
                f"適用済みreplanの履歴がありません: {', '.join(missing_history)}",
            )
        )
    else:
    ...
    try:
        ...
        issues.append(
            _issue("ERROR", f"Command API設定または監査履歴が不正です: {exc}")
        )
    else:
    ...
    try:
        ...
        if all(value in gitignore for value in ignored_runtime_paths):
            ...
            issues.append(
                _issue("WARN", "実行時データまたは個人設定のGit除外を確認できません")
            )
    except OSError:
        issues.append(_issue("WARN", ".gitignoreを読み込めません"))
    config_report = validate_config(root)
    if config_report["valid"]:
        checks.append("unified config")
    else:
        for message in config_report["errors"]:
            issues.append(_issue("ERROR", f"config: {message}"))
    try:
        backups = list_backups(root)
    except (OSError, ValueError) as exc:
        issues.append(_issue("ERROR", f"backup一覧を検証できません: {exc}"))
    else:
        invalid_backups = [item for item in backups if not item.get("verified")]
        if invalid_backups:
            issues.append(
                _issue(
                    "ERROR",
                    f"破損または未検証backupが{len(invalid_backups)}件あります",
                )
            )
        else:
            checks.append("backup manifests")
    try:
        restore_history(root)
    except (OSError, ValueError) as exc:
        issues.append(_issue("ERROR", f"restore履歴が不正です: {exc}"))
    else:
        checks.append("restore history")
    try:
        load_migration_history(root)
        schema_version = current_schema_version(root)
    except (OSError, ValueError) as exc:
        issues.append(_issue("ERROR", f"migration履歴が不正です: {exc}"))
    else:
        checks.append(f"migration schema {schema_version}")
    for label, directory in (
        ("confirmation", root / "data" / "api" / "confirmations"),
        ("idempotency", root / "data" / "api" / "idempotency"),
        ("audit", root / "data" / "api" / "audit"),
    ):
        valid = True
        for path in sorted(directory.glob("*.json")) if directory.is_dir() else []:
            try:
                ...
                issues.append(
                    _issue("ERROR", f"{label} JSONが不正です: {path.name} ({exc})")
                )
                valid = False
        if valid:
            checks.append(f"{label} records")
    try:
        launchd = install_status(root)
    except (OSError, ValueError) as exc:
        issues.append(_issue("WARN", f"launchd状態を確認できません: {exc}"))
    else:
        if launchd["plist_exists"] and not launchd["matches_expected"]:
            issues.append(
                _issue("WARN", "launchd plistが現在の設定・実行パスと一致しません")
            )
        else:
            checks.append("launchd configuration")
    return {
        "root": root,
