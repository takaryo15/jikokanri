...
def release_check(root: Path | None = RootOption) -> None:
    ...
    if not (source_root / "config" / "scheduler.example.json").is_file():
        errors.append("config/scheduler.example.json がありません")
    ...
    for name in ("scheduler.py", "scheduler_install.py", "operational_flows.py"):
        if not (source_root / "src" / "daily_review" / name).is_file():
            errors.append(f"v1.3 schedulerモジュールがありません: {name}")
    ...
    typer.echo("OK   v1.3 scheduler and operational flow foundations")
...
def _structured_output(title: str, value: dict[str, Any] | list[Any], output_format: str) -> None:
...
def scheduler_run_due_command(
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    dry_run: bool = typer.Option(False, "--dry-run", help="履歴・通知・データを書き込みません。"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
...
def scheduler_run_command(
    ...
    force: bool = typer.Option(False, "--force", help="成功済みslotを手動再実行します。"),
    idempotency_key: str | None = typer.Option(None, "--idempotency-key"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
...
def scheduler_doctor_command(
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    repair: bool = typer.Option(False, "--repair", help="stale lockだけを安全に削除します。"),
...
def scheduler_install_command(
    ...
    dry_run: bool = typer.Option(False, "--dry-run", help="登録せず内容だけ表示します。"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
...
def scheduler_uninstall_command(
    dry_run: bool = typer.Option(False, "--dry-run", help="削除せず予定だけ表示します。"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
...
def flow_morning_command(
    ...
    _run_flow_command("morning", root=root, date=date, month=None, at=at, dry_run=dry_run, force=force, output_format=output_format)
...
def flow_nightly_command(
    ...
    _run_flow_command("nightly", root=root, date=date, month=None, at=at, dry_run=dry_run, force=force, output_format=output_format)
...
def flow_weekly_command(
    ...
    _run_flow_command("weekly", root=root, date=date, month=None, at=at, dry_run=dry_run, force=force, output_format=output_format)
...
def flow_monthly_command(
    ...
    _run_flow_command("monthly", root=root, date=None, month=month, at=at, dry_run=dry_run, force=force, output_format=output_format)
