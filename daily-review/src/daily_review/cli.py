...
from .chat_import import (
    backup_unapproved_draft,
    build_draft as build_chat_import_draft,
    import_hash,
)
from .chat_schema import (
    ChatSchemaError,
    extract_json as extract_chat_json,
    validate_payload as validate_chat_payload,
)
from .chat_workflow import (
    build_dynamic_prompt,
    chat_home_next_command,
    load_priorities,
    workflow_state,
)
...
from .goals import (
    ...
    find_step,
    goal_progress,
    goal_summary as build_goal_summary,
    load_goal,
    load_goals,
    milestone_progress,
    milestone_warnings,
...
from .planning import (
    ...
    PlanningError,
    apply_step_updates,
    approve_plan as approve_goal_plan,
    generate_daily_plan,
...
from .evaluation import (
    EvaluationError,
    approve_evaluation,
    generate_monthly_evaluation,
    generate_weekly_evaluation,
    load_monthly_evaluation,
    load_weekly_evaluation,
    save_evaluation,
)
from .replan import (
    ReplanError,
...
from .goal_design import (
    ...
    load_design,
    receive_proposal,
    render_prompt as render_goal_design_prompt,
    save_answer,
)
...
from .quick_review import (
    QuickReviewError,
    build_quick_entry,
    normalize_payload as normalize_quick_payload,
    save_quick_review,
)
...
from .notifications import (
    ...
    load_history as load_notification_history,
...
from .operational_flows import (
    approve_report,
    report_source_revision,
    run_operational_flow,
)
...
replan_app = typer.Typer(
    help="評価に基づく計画修正案を確認して適用します。", invoke_without_command=True
)
design_app = typer.Typer(
    help="ChatGPTと往復する目標設計セッションを管理します。",
    invoke_without_command=True,
)
...
api_app = typer.Typer(
    help="ChatGPTや外部プログラム向けのversioned JSON Command APIです。"
)
parse_app = typer.Typer(help="自然言語を安全な構造へルールベースで正規化します。")
backup_app = typer.Typer(
    help="検証可能なZIPバックアップを作成・管理します。", invoke_without_command=True
)
rollover_app = typer.Typer(help="未完了タスクを複製せず翌日の計画へ引き継ぎます。")
doctor_app = typer.Typer(
    help="保存構造の点検と安全な修復を行います。", invoke_without_command=True
)
scheduler_app = typer.Typer(
    help="設定された時刻を内部判定し、dueなjobだけを安全に実行します。"
)
flow_app = typer.Typer(
    help="朝・夜・週次・月次の運用を承認境界を保ったまままとめて実行します。"
)
config_app = typer.Typer(help="統合設定の表示、検証、保存場所確認を行います。")
migrate_app = typer.Typer(
    help="既存データを保持したままschema migrationを確認・適用します。",
    invoke_without_command=True,
)
...
def main(
    version: bool = typer.Option(
        False, "--version", help="バージョンを表示して終了します。", is_eager=True
    ),
) -> None:
    if version:
...
RootOption = typer.Option(
    None, "--root", help="保存先ルート。未指定ならカレントディレクトリです。"
)
DateOption = typer.Option(
    None, "--date", help="対象日（YYYY-MM-DD）。未指定なら今日です。"
)
...
def _read_natural_input(text: str | None, clipboard: bool) -> tuple[str, str]:
    ...
    if text is not None:
        if _stdin_is_piped() and sys.stdin.read():
            raise typer.BadParameter(
                "--text、--clipboard、標準入力は同時に使用できません"
            )
        return text, "text"
    if clipboard:
        if _stdin_is_piped() and sys.stdin.read():
...
def _read_clipboard_text() -> str:
    if platform.system() != "Darwin":
        raise typer.BadParameter(
            "クリップボード入力はmacOSのみ対応しています。--file または標準入力を使ってください。"
        )
    try:
        result = subprocess.run(["pbpaste"], check=True, capture_output=True, text=True)
    except (OSError, subprocess.CalledProcessError) as exc:
        raise typer.BadParameter(
            "クリップボードを読み取れませんでした。--file または標準入力を使ってください。"
        ) from exc
    if not result.stdout.strip():
        raise typer.BadParameter(
            "クリップボードが空です。ChatGPTのJSONをコピーしてから再実行してください。"
        )
...
def _strip_single_json_code_block(text: str, source: str) -> str:
    ...
    if (
        len(fence_indexes) != 2
        or fence_indexes[0] != 0
        or fence_indexes[1] != len(lines) - 1
    ):
        raise typer.BadParameter(
    ...
    if opening not in {"```", "```json"}:
        raise typer.BadParameter(
            f"{source} のコードブロック種別はjsonだけ対応しています。"
        )
    return "\n".join(lines[1:-1])
...
def _read_json_for_command(file: Path | None, clipboard: bool) -> dict[str, Any]:
    if file and clipboard:
        raise typer.BadParameter(
            "--file と --clipboard は同時に指定できません。どちらか一方だけ使ってください。"
        )
    if clipboard:
...
def _print_save_error(
    problem: str, fix: str = "JSONを修正してから、もう一度dry-runしてください。"
) -> None:
...
def _resolve_night_date(cli_date: str | None, payload: dict[str, Any]) -> str:
    ...
    if cli_date and json_date and cli_date != json_date:
        raise typer.BadParameter(
            f"--date とJSON内のdateが一致しません（--date: {cli_date}, JSON: {json_date}）"
        )
    ...
    try:
        ...
        raise typer.BadParameter(
            f"dateはYYYY-MM-DD形式にしてください（現在: {day}）"
        ) from exc
    return day


def _build_pending_plan(
    payload: dict[str, Any], day: str
...
def _print_task_results(
    target: str, entry: dict[str, Any], plan: dict[str, Any]
) -> None:
    ...
    for index, task in enumerate(tasks, start=1):
        ...
        typer.echo(f"{index}. [{task.get('area')}] {task.get('task')}")
        typer.echo(
            f"   結果: {_task_result_label(result.get('status') if result else None)}"
        )
        typer.echo(
            f"   最低ライン: {_minimum_label(result.get('minimum_line_achieved') if result else None)}"
        )
...
def _find_final_entry_by_target(
    root: Path, target_date: str
...
def _validate_task_results_payload(
    results: list[dict[str, Any]], plan: dict[str, Any]
) -> tuple[list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
    task_ids = [
        str(task.get("id")) for task in plan.get("tasks") or [] if task.get("id")
    ]
    ...
    duplicated = sorted(
        {task_id for task_id in input_ids if input_ids.count(task_id) > 1}
    )
    if duplicated:
    ...
    for result in results:
        ...
        if status == "minimum_only" and achieved is False:
            warnings.append(
                f"{task_id}: 最低ラインのみですが最低ライン未達になっています"
            )
...
def _prepare_close_day(
    ...
    if not isinstance(proposal_payload, dict):
        raise typer.BadParameter(
            "tomorrow_plan_proposalはJSONオブジェクトにしてください。"
        )
    ...
    if has_task_results and raw_task_results:
        ...
        if not source_entry or not source_plan or not source_day:
            if _has_pending_by_target(root, day):
                raise typer.BadParameter(
                    f"{day} は提案版のみです。承認後にclose-dayを実行してください。"
                )
        ...
        source_entry["task_results"] = _merge_task_results(
            source_entry.get("task_results") or [], updates
        )
        ...
        warnings.append(
            "当日のタスク結果は保存されませんでした（task_resultsが空です）。"
        )
    else:
...
def _print_close_day_dry_run(
    ...
    target = (entries_by_day[day].get("tomorrow_plan_proposal") or {}).get(
        "target_date", "-"
    )
...
def _print_close_day_summary(
    ...
    typer.echo(
        f"翌日の提案版       {_saved_label(entry.get('tomorrow_plan_proposal'))}"
    )
    typer.echo(
        f"翌日の確定版       {'承認済み' if final and final.get('status') == 'approved' else '未承認'}"
    )
...
def _print_night_summary(day: str, entry: dict[str, Any], warnings: list[str]) -> None:
    ...
    typer.echo(
        f"確定版       {'承認済み' if final and final.get('status') == 'approved' else '未承認'}"
    )
...
def config_show_command(
    ...
    json_output: bool = typer.Option(False, "--json", help="JSONだけを表示します。"),
) -> None:
    """既定値を補った有効設定を表示します。秘密値は設定対象外です。"""
    base = _root(root)
    try:
        value = load_effective_config(base)
    except (OSError, ValueError) as exc:
        typer.echo(f"ERROR: 設定を読み込めません: {exc}", err=True)
        raise typer.Exit(code=3) from exc
    if json_output:
        typer.echo(json.dumps(value, ensure_ascii=False, indent=2))
        return
    typer.echo(f"daily-review config｜{base}")
    typer.echo(f"timezone: {value['app']['timezone']}")
    typer.echo(f"週開始: {value['app']['week_start']}")
    typer.echo(f"Main上限: {value['app']['main_limit']}")
    typer.echo(
        f"scheduler: {'有効' if value['scheduler']['enabled'] else '無効（安全な既定値）'}"
    )
    typer.echo(f"backup先: {value['backup']['directory']}")
    typer.echo("restore前backup: 有効")
...
def config_validate_command(
    ...
    """すべての設定を型・範囲・timezoneまで検証します。"""
    result = validate_config(_root(root))
    if json_output:
        typer.echo(json.dumps(result, ensure_ascii=False, indent=2))
    elif result["valid"]:
        typer.echo("daily-review config validate: OK")
    else:
        for error in result["errors"]:
            typer.echo(f"ERROR: {error}", err=True)
        typer.echo("daily-review config validate: ERROR")
    if not result["valid"]:
...
def config_path_command(
    ...
    """設定ディレクトリと統合app設定の保存先を表示します。"""
    base = _root(root)
    value = {
        "directory": str(config_directory(base)),
        "app": str(app_config_path(base)),
    }
    typer.echo(
        json.dumps(value, ensure_ascii=False, indent=2)
        if json_output
        else f"設定ディレクトリ: {value['directory']}\napp設定: {value['app']}"
    )


def _interactive_setup() -> dict[str, Any]:
    if not typer.confirm("週は火曜日開始・月曜日終了でよいですか？", default=True):
        raise ConfigurationError("v1.3の週開始日は火曜日です")
    if not typer.confirm("Mainは最大3件でよいですか？", default=True):
        raise ConfigurationError("v1.3のMain上限は3件です")
    typer.echo("最低限ラインは、体調不良日にも実行できる最小の一歩です。")
    timezone = typer.prompt("timezone", default="Asia/Tokyo")
    night = typer.prompt("夜の通知時刻", default="21:00")
    morning = typer.prompt("朝の確認時刻", default="07:30")
    scheduler_enabled = typer.confirm(
        "schedulerを設定上有効にしますか？", default=False
    )
    launchd_install = scheduler_enabled and typer.confirm(
        "設定保存後にlaunchdへschedulerを導入しますか？", default=False
    )
    console_sender = typer.confirm("console通知を使いますか？", default=True)
    file_sender = typer.confirm("file通知を使いますか？", default=True)
    quiet_enabled = typer.confirm("quiet hoursを有効にしますか？", default=False)
    quiet_start = typer.prompt("quiet hours開始", default="00:00")
    quiet_end = typer.prompt("quiet hours終了", default="07:00")
    backup_directory = typer.prompt("backup保存先", default="backups")
    priorities = typer.prompt(
        "カテゴリ優先順位（カンマ区切り）",
        default=",".join(DEFAULT_PRIORITIES["priorities"]),
    )
    return {
        "app": {
            "timezone": timezone,
        ...
        "priorities": {
            "priorities": [
                item.strip() for item in priorities.split(",") if item.strip()
            ]
        },
        "notifications": {
            "console": {"enabled": console_sender},
            "file": {"enabled": file_sender},
            "reminder_review": {"enabled": True, "time": night},
            "quiet_hours": {
        ...
        "scheduler": {
            ...
            "timezone": timezone,
            "jobs": {
                "review_reminder": {"time": night},
                "morning_instruction_check": {"time": morning},
            },
        },
        "recovery": {
            "backup": {
...
def setup_command(
    config_file: Path | None = typer.Option(
        None, "--config", help="non-interactive setup JSON。"
    ),
    root: Path | None = RootOption,
    dry_run: bool = typer.Option(
        False, "--dry-run", help="設定を書き込まずpreviewだけを表示します。"
    ),
    yes: bool = typer.Option(False, "--yes", help="preview確認済みとして保存します。"),
) -> None:
    """初回設定をpreviewし、既存設定を上書きせず保存します。"""
    base = _root(root)
    try:
        if config_file:
            value = read_json_file(config_file.expanduser())
        elif sys.stdin.isatty():
            selected_root = typer.prompt("データ保存先", default=str(base))
            base = resolve_root(Path(selected_root))
            value = _interactive_setup()
        else:
            value = {"app": {}}
        payload = normalize_setup_payload(value)
        plan = setup_plan(base, payload)
    except (OSError, ValueError, ConfigurationError) as exc:
        typer.echo(f"ERROR: setup設定が不正です: {exc}", err=True)
        raise typer.Exit(code=2) from exc
    typer.echo("daily-review setup｜preview")
    typer.echo(f"保存先ルート: {base}")
    typer.echo("週開始: tuesday")
    typer.echo("Main上限: 3")
    typer.echo(f"scheduler: {'有効' if plan['scheduler_enabled'] else '無効'}")
    typer.echo(
        "launchd: "
        + ("導入予定" if plan["launchd_install"] else "変更しません（安全な既定値）")
    )
    for item in plan["files"]:
        typer.echo(f"- {item['path']}: {item['action']}")
    if plan["conflicts"]:
        typer.echo("ERROR: 既存設定は上書きしません", err=True)
        typer.echo("daily-review config show で確認してください", err=True)
        if not dry_run:
            raise typer.Exit(code=3)
    if dry_run:
        typer.echo("設定は保存していません")
        return
    if not yes and not typer.confirm("この新規設定を保存しますか？", default=False):
        typer.echo("setupを中止しました")
        raise typer.Exit(code=2)
    writes = []
    for item in plan["files"]:
        if item["action"] != "create":
            continue
        writes.append((Path(item["path"]), item["settings"]))
    try:
        atomic_write_json_data_many(writes)
        launchd_result = install_scheduler(base) if plan["launchd_install"] else None
    except (OSError, ValueError) as exc:
        typer.echo(f"ERROR: 設定を保存できません: {exc}", err=True)
        raise typer.Exit(code=4) from exc
    typer.echo(f"設定を保存しました: {len(writes)}件")
    if launchd_result:
        typer.echo(f"launchdを導入しました: {launchd_result['plist_path']}")
    else:
        typer.echo("launchdを使う場合: daily-review scheduler install --dry-run")
    typer.echo("次の操作: daily-review doctor")


@app.command("onboarding")
def onboarding_command(root: Path | None = RootOption) -> None:
    """初日の準備状況と安全な次の操作を読み取り専用で表示します。"""
    base = _root(root)
    config_result = validate_config(base)
    doctor_result = run_doctor(base)
    doctor_errors = [
        item for item in doctor_result["issues"] if item["level"] == "ERROR"
    ]
    try:
        scheduler_result = scheduler_status(base, _scheduler_now(base, None))
        scheduler_label = (
            "有効" if scheduler_result.get("enabled") else "無効（未導入）"
        )
    except (OSError, ValueError, SchedulerError):
        scheduler_label = "設定エラー"
    typer.echo("daily-review onboarding")
    typer.echo(f"package: daily-review {__version__}")
    typer.echo(f"data root: {base}")
    typer.echo(f"config: {'OK' if config_result['valid'] else 'ERROR'}")
    typer.echo(f"doctor: {'OK' if not doctor_errors else 'ERROR'}")
    typer.echo(f"scheduler: {scheduler_label}")
    typer.echo(f"backup directory: {base / 'backups'}")
    typer.echo("初日の確認:")
    typer.echo("daily-review doctor")
    typer.echo("daily-review close-day --date YYYY-MM-DD --clipboard --dry-run")
    typer.echo("daily-review home --date YYYY-MM-DD")
...
def migrate(
    ...
    """サブコマンドなしでは従来互換のmigrationを実行します。"""
...
def migrate_check_command(
    ...
    """現在schemaとv1.3移行予定を読み取り専用で確認します。"""
    base = _root(root)
    try:
        result = migration_check(base)
    except (OSError, ValueError) as exc:
        typer.echo(f"ERROR: migrationを確認できません: {exc}", err=True)
        raise typer.Exit(code=3) from exc
    if json_output:
        typer.echo(json.dumps(result, ensure_ascii=False, indent=2))
        return
    typer.echo("daily-review migrate check")
    typer.echo(f"現在schema: {result['source_schema_version']}")
    typer.echo(f"移行先schema: {result['target_schema_version']}")
    typer.echo(f"対象ファイル: {len(result['target_files'])}件")
    typer.echo("既存の日次・週次・月次データ: 変更なし")
    typer.echo("migration前backup: 必須")
    for item in result["manual_checks"]:
        typer.echo(f"確認: {item}")
...
def migrate_apply_command(
    root: Path | None = RootOption,
    dry_run: bool = typer.Option(
        False, "--dry-run", help="変更せず予定だけを表示します。"
    ),
    yes: bool = typer.Option(False, "--yes", help="確認済みとして実行します。"),
    json_output: bool = typer.Option(False, "--json", help="JSONだけを表示します。"),
) -> None:
    """検証済みフルバックアップ後に不足項目だけを追加します。"""
    base = _root(root)
    try:
        check = migration_check(base)
    except (OSError, ValueError) as exc:
        typer.echo(f"ERROR: migrationを確認できません: {exc}", err=True)
        raise typer.Exit(code=3) from exc
    if dry_run:
        payload = {**check, "status": "dry_run", "written": False}
        if json_output:
            typer.echo(json.dumps(payload, ensure_ascii=False, indent=2))
        else:
            typer.echo("daily-review migrate apply｜dry-run")
            typer.echo(f"現在schema: {check['source_schema_version']}")
            typer.echo(f"移行先schema: {check['target_schema_version']}")
            for item in check["plan"]:
                typer.echo(
                    f"- {item['path']}: "
                    f"{'作成予定' if item['action'] == 'create' else '既存'}"
                )
            typer.echo("backup・migrationともに実行していません")
        return
    if check["already_migrated"] and not check["target_files"]:
        payload = {"status": "already_migrated", "changes": [], **check}
        typer.echo(
            json.dumps(payload, ensure_ascii=False, indent=2)
            if json_output
            else "v1.3 schemaへのmigrationは完了済みです"
        )
        return
    if not yes and not typer.confirm(
        "検証済みbackupを作成してmigrationを実行しますか？", default=False
    ):
        typer.echo("migrationを中止しました")
        raise typer.Exit(code=2)
    try:
        backup_path, manifest = create_backup(base, manual=False)
        verified = verify_backup(backup_path)
        result = apply_migration(base)
        doctor_result = run_doctor(base)
    except (OSError, ValueError, OperationLockedError) as exc:
        typer.echo(f"ERROR: migrationに失敗しました: {exc}", err=True)
        raise typer.Exit(code=4) from exc
    errors = [item for item in doctor_result["issues"] if item["level"] == "ERROR"]
    payload = {
        "status": "applied" if not errors else "applied_with_preexisting_errors",
        "backup_id": manifest["backup_id"],
        "backup_path": str(backup_path),
        "backup_verified": verified["valid"],
    ...
    if json_output:
        ...
        typer.echo("v1.3 migrationを適用しました")
        typer.echo(f"backup ID: {manifest['backup_id']}")
        typer.echo("backup検証: OK")
        typer.echo(f"変更: {len(result['changes'])}件")
        typer.echo(f"doctor ERROR: {len(errors)}件")
        typer.echo("次の操作: daily-review doctor")
    if errors:
        raise typer.Exit(code=3)
...
def v12_check(
    ...
    if json_output:
        ...
        if report["errors"]:
            ...
            typer.echo(
                f"v{report['version']} is ready"
                if report["version"] == "1.2.0"
                else "v1.2.0rc1 is ready for final operational testing"
            )
    if report["errors"]:
        raise typer.Exit(code=5)
...
def tasks_list(
    status: str | None = typer.Option(
        None,
        "--status",
        help="pending / completed / partial / minimum_only / not_started / skipped",
    ),
    ...
    minimum_only: bool = typer.Option(
        False, "--minimum", help="最低限ライン付きだけ表示"
    ),
    ...
    if output_format == "json":
        typer.echo(
            json.dumps(
                {"date": day, "count": len(values), "tasks": values},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    for index, item in enumerate(values, start=1):
        ...
        typer.echo(
            f"   ID: {item['short_id']} / 優先度: {item['priority']} / カテゴリ: {item['category'] or '未設定'} / 期限: {item['due_date'] or '未設定'}"
            + (f" / {', '.join(flags)}" if flags else "")
        )
        if detail:
            typer.echo(
                f"   参照元: {item['source']} {item['source_review_date']} / 作成: {item['created_at'] or '未記録'} / 更新: {item['updated_at'] or '未記録'}"
            )
...
def export_csv_command(
    kind: str = typer.Option(
        "all", "--type", help="reviews / tasks / instructions / all"
    ),
    output: Path | None = typer.Option(
        None, "--output", help="CSVファイルまたはall用ディレクトリ"
    ),
    date: str | None = typer.Option(
        None, "--date", help="単日指定。--periodの基準日にも使用"
    ),
    ...
    period: str | None = typer.Option(
        None, "--period", help="week（火曜開始）または month"
    ),
    ...
    try:
        start, end = period_range(
            day=date, date_from=date_from, date_to=date_to, period=period
        )
        result = export_csv(
            _root(root),
            kind=kind,
...
def notifications_check(
    ...
    dry_run: bool = typer.Option(
        False, "--dry-run", help="候補表示のみで送信・履歴保存を行わない"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if dry_run:
        typer.echo(
            f"dry-run: {len(values)}件の候補を表示し、送信・履歴保存は行いませんでした。"
        )
        return
    senders = []
    ...
    try:
        result = dispatch_notifications(
            base, values, current=current, config=config, senders=senders
        )
    except (NotificationError, OSError, ValueError) as exc:
        typer.echo(f"ERROR: 通知履歴を保存できません: {exc}", err=True)
        raise typer.Exit(code=4) from exc
    typer.echo(
        f"送信: {result['sent']}件 / 重複スキップ: {result['skipped']}件 / 失敗: {result['failed']}件"
    )
    if result["failed"]:
        typer.echo(
            "WARN: 一部通知に失敗しました。日次データは変更していません。", err=True
        )
...
def notifications_history(
    ...
    try:
        values = list(reversed(load_notification_history(_root(root))["records"]))[
            :limit
        ]
    except (NotificationError, OSError, ValueError) as exc:
    ...
    for item in values:
        typer.echo(
            f"- {item.get('attempted_at', '未記録')} [{item.get('status', '不明')}] {item.get('notification_type', '不明')} -> {item.get('destination', '不明')}"
        )
...
def api_execute(
    input_path: Path | None = typer.Option(
        None, "--input", help="CommandRequest JSONファイル"
    ),
    stdin: bool = typer.Option(False, "--stdin", help="標準入力からJSONを読む"),
    mode: str | None = typer.Option(
        None, "--mode", help="requestのmodeをpreviewまたはcommitで上書き"
    ),
    confirmation_token: str | None = typer.Option(
        None, "--confirmation-token", help="previewで発行された確認トークン"
    ),
    ...
    try:
        ...
        response = CommandResponse(
            ...
            errors=[
                {"code": "INVALID_REQUEST", "message": str(exc), "recoverable": True}
            ],
            metadata={},
        )
...
def api_schema(
    ...
    pretty: bool = typer.Option(
        True, "--pretty/--compact", help="整形JSONまたはcompact JSON"
    ),
) -> None:
    ...
    if command:
                ...
                json.dumps(
                    {
                        "error": {
                            ...
                            "message": f"不明なcommandです: {command}",
                        }
                    },
                    ensure_ascii=False,
                )
        ...
        value = {
            "version": "1",
            "request": CommandRequest.model_json_schema(),
            "response": CommandResponse.model_json_schema(),
        }
    else:
        typer.echo(
            json.dumps(
                {
                    "error": {
                        ...
                        "message": "--typeはrequest、response、allにしてください",
                    }
                },
                ensure_ascii=False,
            )
...
def api_history(
    ...
    try:
        ...
        output = {
            "count": 0,
            "records": [],
            "errors": [{"code": "STORAGE_ERROR", "message": str(exc)}],
        }
...
def parse_review_command(
    ...
    preview: bool = typer.Option(
        False, "--preview", help="正規化結果をCommand APIでpreview"
    ),
    commit: bool = typer.Option(
        False, "--commit", help="confirmation tokenを使って確定"
    ),
    ...
    stable_key = (
        idempotency_key
        or f"parse-review-{normalized['date']}-{hashlib.sha256(raw_text.encode()).hexdigest()[:12]}"
    )
    request = {
        "version": "1",
...
def _print_goal_warnings(goal: dict[str, Any], goals: list[dict[str, Any]]) -> None:
    ...
    if (
        parent
        and isinstance(due_date, str)
        and isinstance(parent.get("due_date"), str)
        and due_date > parent["due_date"]
    ):
...
def _print_goal(goal: dict[str, Any], goals: list[dict[str, Any]]) -> None:
    ...
    typer.echo(
        f"期間: {goal.get('start_date') or '未設定'}〜{goal.get('due_date') or '未設定'}"
    )
    ...
    for item in goal.get("qualitative_criteria") or []:
        typer.echo(
            f"- [{labels.get(item.get('status'), '不正')}] {item.get('description', '未設定')}"
        )
    ...
    for item in goal.get("quantitative_metrics") or []:
        typer.echo(
            f"- {item.get('name', '未設定')}: {item.get('current')} / {item.get('target')}{item.get('unit', '')}"
        )
...
def goal_design_prompt(
    design_id: str = typer.Argument(...), root: Path | None = RootOption
) -> None:
...
def goal_add(
    ...
    level: str | None = typer.Option(
        None, "--level", help="vision / long / medium / short"
    ),
    ...
    qualitative: list[str] = typer.Option(
        [], "--qualitative", help="達成を判断できる定性指標。複数指定可。"
    ),
    metric: list[str] = typer.Option(
        [], "--metric", help="name|unit|baseline|target|direction。複数指定可。"
    ),
    root: Path | None = RootOption,
) -> None:
    """目標を作成します。"""
...
def goal_list(
    ...
    if due_before is not None:
        result = [
            goal
            for goal in result
            if isinstance(goal.get("due_date"), str) and goal["due_date"] <= due_before
        ]
    result.sort(
        key=lambda goal: (
            goal.get("due_date") is None,
            goal.get("due_date") or "",
            goal.get("title", ""),
        )
    )
    if json_output:
...
def goal_archive(
    ...
    try:
        ...
        _goal_error(str(exc), code=3)
    if not yes and not typer.confirm(
        f"目標「{goal['title']}」をアーカイブしますか？", default=False
    ):
...
def _confirm_roadmap_warnings(warnings: list[str], *, allow_warning: bool) -> None:
    ...
    if not allow_warning and not typer.confirm(
        "警告を確認して保存しますか？", default=False
    ):
...
def _print_milestone(milestone: dict[str, Any]) -> None:
    ...
    typer.echo(
        f"期間: {milestone.get('start_date') or '未設定'}〜{milestone.get('due_date') or '未設定'}"
    )
    ...
    for item in milestone.get("quantitative_metrics") or []:
        typer.echo(
            f"- {item['name']}: {item['current']} / {item['target']}{item['unit']}"
        )
...
def milestone_add(
    ...
    try:
        ...
        _confirm_roadmap_warnings(
            milestone_warnings(goal, milestone), allow_warning=allow_warning
        )
...
def milestone_list(
    ...
    if due_before:
        milestones = [
            ...
            if item.get("due_date") and item["due_date"] <= due_before
        ]
    if json_output:
        typer.echo(
            json.dumps(
                {"goal_id": goal_id, "milestones": milestones},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    for milestone in milestones:
        typer.echo(
            f"{milestone['order']}. [{milestone['status']}] {milestone['title']}"
        )
        typer.echo(f"   期限: {milestone.get('due_date') or '未設定'}")
        typer.echo(f"   進捗: {_milestone_progress_label(milestone)}")
        done = sum(
            step.get("status") == "done" for step in milestone.get("steps") or []
        )
...
def milestone_show(
    ...
    try:
        ...
        _goal_error(str(exc), code=3)
    if json_output:
        typer.echo(json.dumps(milestone, ensure_ascii=False, indent=2))
        return
...
def milestone_edit(
    ...
    clear_qualitative: bool = typer.Option(False, "--clear-qualitative"),
    clear_metrics: bool = typer.Option(False, "--clear-metrics"),
    depends_on: list[str] = typer.Option([], "--depends-on"),
    clear_dependencies: bool = typer.Option(False, "--clear-dependencies"),
    allow_warning: bool = typer.Option(False, "--allow-warning"),
    root: Path | None = RootOption,
) -> None:
    """マイルストーンを編集します。"""
    ...
    try:
        ...
        preview = dict(milestone)
        preview.update(changes)
        _confirm_roadmap_warnings(
            milestone_warnings(goal, preview), allow_warning=allow_warning
        )
...
def milestone_status(
    ...
    try:
        ...
        pending = [
            step
            for step in milestone.get("steps") or []
            if step.get("status") not in {"done", "cancelled"}
        ]
        ...
        if status == "completed":
            changes["completed_at"] = milestone.get("completed_at") or now_iso()
        elif milestone.get("completed_at") is not None:
...
def milestone_reorder(
    ...
    try:
        reorder_milestone(
            _root(root), goal_id, milestone_id, before_id=before, position=position
        )
    except (GoalError, OSError, ValueError) as exc:
        _goal_error(str(exc), code=3)
...
def step_add(
    ...
    try:
        ...
        step = new_step(
            ...
            due_date=due_date,
            minimum=minimum,
            order=len(milestone.get("steps") or []) + 1,
        )
        _confirm_roadmap_warnings(
            step_warnings(milestone, step), allow_warning=allow_warning
        )
...
def step_list(
    ...
    try:
        ...
        _goal_error(str(exc), code=3)
    if json_output:
        typer.echo(json.dumps({"steps": steps}, ensure_ascii=False, indent=2))
        return
...
def step_edit(
    ...
    try:
        goal = load_goal(base, goal_id)
        milestone, step = find_step(goal, milestone_id, step_id)
        changes = {
            key: value
            for key, value in {
                "title": title,
        ...
        preview.update(changes)
        _confirm_roadmap_warnings(
            step_warnings(milestone, preview), allow_warning=allow_warning
        )
...
def step_status(
    ...
    if status not in STEP_STATUSES:
        _goal_error("ステップstatusが不正です")
    try:
        base = _root(root)
        _, step = find_step(load_goal(base, goal_id), milestone_id, step_id)
        changes: dict[str, Any] = {"status": status}
        if status == "done":
            changes["completed_at"] = step.get("completed_at") or now_iso()
        elif step.get("completed_at") is not None:
            changes["completed_at"] = None
...
def goal_roadmap(
    ...
    if json_output:
        typer.echo(
            json.dumps(
                {"goal": goal, "milestones": milestones}, ensure_ascii=False, indent=2
            )
        )
        return
    ...
    for milestone in milestones:
        ...
        if (
            ...
            and milestone["due_date"] < today_string()
            and milestone.get("status") not in {"completed", "cancelled"}
        ):
            due_label += "｜期限超過"
        typer.echo(due_label)
        typer.echo(
            f"├─ [{milestone['status']}] {milestone['title']}｜{_milestone_progress_label(milestone)}"
        )
        if not compact:
            for step in sorted(
                milestone.get("steps") or [], key=lambda item: item["order"]
            ):
                typer.echo(f"│  ├─ [{step['status']}] {step['title']}")
...
def goal_next(
    ...
    try:
        goal = load_goal(_root(root), goal_id)
        action = next_goal_action(goal, today=_day(date))
    except (GoalError, OSError, ValueError) as exc:
        _goal_error(str(exc), code=3)
    if not action:
    ...
    if step:
        ...
        typer.echo(
            f"期限: {step.get('due_date') or milestone.get('due_date') or '未設定'}"
        )
...
def _print_week_plan(plan: dict[str, Any], *, saved: bool) -> None:
    ...
    if plan.get("carryovers"):
        typer.echo("前週からの繰越:")
        for item in plan["carryovers"]:
            typer.echo(f"- {item.get('title', item)}")
    typer.echo(
        f"保存状態: {'承認済み' if plan.get('status') == 'approved' else '保存済み' if saved else '未保存'}"
    )
    if not saved:
        typer.echo(
            f"次の操作: daily-review plan week --date {plan['week_start']} --save"
        )
...
def _print_daily_plan(plan: dict[str, Any], *, saved: bool) -> None:
    ...
    for index, item in enumerate(plan.get("main_candidates") or [], start=1):
        typer.echo(f"{index}. {item['category']}｜{item['title']}")
        if item.get("minimum"):
            typer.echo(f"   最低ライン: {item['minimum']}")
        typer.echo(f"   理由: {item.get('reason') or '目標の次アクション'}")
    if not plan.get("main_candidates"):
        typer.echo("なし")
    if plan.get("other_tasks"):
        typer.echo("その他候補:")
        for item in plan["other_tasks"]:
            typer.echo(f"- {item['title']}")
    if len(plan.get("main_candidates") or []) >= MAX_MAIN_CANDIDATES and plan.get(
        "other_tasks"
    ):
        typer.echo(
            "WARNING: 今日の候補が多すぎます。Mainは最大3件に絞ってください", err=True
        )
    if not saved:
...
def plan_week(
    ...
    if dry_run:
        typer.echo("dry-run: 保存は行いませんでした")
...
def plan_review(
    ...
    date: str | None = DateOption,
    json_output: bool = typer.Option(False, "--json"),
    remove: str | None = typer.Option(None, "--remove"),
    add_step: tuple[str, str, str] | None = typer.Option(None, "--add-step"),
    move: str | None = typer.Option(None, "--move"),
    position: int | None = typer.Option(None, "--position"),
    set_minimum: str | None = typer.Option(None, "--set-minimum"),
    root: Path | None = RootOption,
) -> None:
    """保存済み週次または日次計画を表示・編集します。"""
    if (week is None) == (date is None):
        _planning_error("--week または --date のどちらか一方を指定してください", code=2)
    base = _root(root)
    try:
        if week:
            ...
            if remove:
                ...
                if len(filtered) == len(items):
                    raise PlanningError("削除対象の重点が見つかりません")
                plan["focus_items"] = filtered
                items = filtered
                changed = True
            if add_step:
                ...
                items.append(_candidate(goal, milestone, step, reason="手動追加"))
                changed = True
            if move:
                if position is None:
                    raise PlanningError("--moveには--positionが必要です")
                item = next((value for value in items if value["id"] == move), None)
                if not item or not 1 <= position <= len(items):
                    raise PlanningError("並べ替え対象またはpositionが不正です")
                items.remove(item)
                items.insert(position - 1, item)
                changed = True
            if changed:
                update_weekly_plan(base, plan)
            if json_output:
                typer.echo(json.dumps(plan, ensure_ascii=False, indent=2))
                return
            ...
            changed = False
            if set_minimum:
                if "=" not in set_minimum:
                    raise PlanningError(
                        "--set-minimumはdaily-plan-xxxxxxxx=最低ライン形式にしてください"
                    )
                item_id, minimum = set_minimum.split("=", 1)
                item = next(
                    (
                        value
                        for value in plan["main_candidates"]
                        if value["id"] == item_id
                    ),
                    None,
                )
                if not item or not minimum.strip():
                    raise PlanningError("最低ラインの対象または内容が不正です")
...
def plan_apply(
    ...
    if (week is None) == (date is None):
        _planning_error("--week または --date のどちらか一方を指定してください", code=2)
    if not yes and not typer.confirm("この計画を承認しますか？", default=False):
        _planning_error("承認を中止しました", code=2)
    try:
        plan = approve_goal_plan(
            _root(root), week=week, day=_day(date) if date else None
        )
...
def goal_link(
    date: str | None = DateOption,
    week: str | None = typer.Option(None, "--week"),
    main_index: int | None = typer.Option(None, "--main-index"),
    task_index: int | None = typer.Option(None, "--task-index"),
    focus_index: int | None = typer.Option(None, "--focus-index"),
    ...
    if (week is None) == (date is None) or sum(
        value is not None for value in (main_index, task_index, focus_index)
    ) != 1:
    ...
    try:
        if week:
            ...
            if (
                not plan
                or focus_index is None
                or not 1 <= focus_index <= len(plan["focus_items"])
            ):
                raise PlanningError("週次重点が見つかりません")
            item = plan["focus_items"][focus_index - 1]
            if goal:
                item["goal_id"] = goal
            if milestone:
                item["milestone_id"] = milestone
            if step:
                item["step_id"] = step
            ...
            record_type, record_index = (
                ("main", main_index) if main_index is not None else ("task", task_index)
            )
            source_items = (
                plan["main_candidates"]
                if record_type == "main"
                else plan.get("other_tasks") or []
            )
            if record_index is None or not 1 <= record_index <= len(source_items):
                raise PlanningError("日次計画項目が見つかりません")
            ...
            plan["goal_links"] = [
                ...
                if (link.get("record_type"), link.get("record_index"))
                != (record_type, record_index)
            ]
            plan["goal_links"].append(
                {
...
def goal_unlink(
    date: str | None = DateOption,
    main_index: int = typer.Option(..., "--main-index"),
    root: Path | None = RootOption,
) -> None:
    ...
    try:
        ...
        if not plan:
            raise PlanningError("日次計画が見つかりません")
        old = len(plan.get("goal_links") or [])
        plan["goal_links"] = [
            link
            for link in plan.get("goal_links") or []
            if link.get("record_index") != main_index
        ]
        if old == len(plan["goal_links"]):
            raise PlanningError("解除対象のリンクが見つかりません")
...
def goal_progress_command(
    ...
    try:
        ...
        for link in plan.get("goal_links") or []:
            ...
            proposed = {"完了": "done", "一部進んだ": "doing", "中止": "cancelled"}.get(
                review_status
            )
    ...
    if not candidates:
        typer.echo("反映候補はありません")
    for index, (link, status) in enumerate(candidates, start=1):
        item = plan["main_candidates"][link["record_index"] - 1]
        typer.echo(f"{index}. {item['title']} → step status: {status}")
    if apply:
        if not yes and not typer.confirm(
            "表示した進捗候補を反映しますか？", default=False
        ):
            _planning_error("進捗反映を中止しました", code=2)
        try:
            apply_step_updates(
                base,
                [
                    (link["goal_id"], link["milestone_id"], link["step_id"], status)
...
def _print_evaluation(value: dict[str, Any], *, period_type: str) -> None:
    ...
    planned, completed = (
        summary.get("planned_main_count", 0),
        summary.get("completed_main_count", 0),
    )
    ...
    for index, item in enumerate(value.get("goal_evaluations") or [], start=1):
        ...
        typer.echo(
            f"   進捗: {item.get('start_progress', 0):g}% → {item.get('end_progress', 0):g}%"
        )
        typer.echo(
            f"   完了step: {item.get('completed_steps', 0)}件 / overdue: {item.get('overdue_steps', 0)}件"
        )
    if not value.get("goal_evaluations"):
        typer.echo("なし")
    typer.echo("診断:")
    for item in value.get("diagnostics") or []:
        typer.echo(f"- {item['message']}")
    if not value.get("diagnostics"):
        typer.echo("なし")
    typer.echo(
        f"保存状態: {'承認済み' if value.get('status') == 'approved' else 'ドラフト'}"
    )
...
def goal_evaluate_week(
    ...
    if save and dry_run:
        _evaluation_error("--save と --dry-run は同時に指定できません", code=2)
    try:
        value = generate_weekly_evaluation(_root(root), _day(date))
        if save and not dry_run:
            save_evaluation(_root(root), value, period_type="week")
...
def goal_evaluate_month(
    ...
    """暦月単位で目標の進捗傾向を評価します。"""
    if month and date:
        _evaluation_error("--month と --date は同時に指定できません", code=2)
    if save and dry_run:
        _evaluation_error("--save と --dry-run は同時に指定できません", code=2)
    ...
    try:
        ...
        if save and not dry_run:
            save_evaluation(_root(root), value, period_type="month")
    except (EvaluationError, PlanningError, GoalError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
    if json_output:
        typer.echo(json.dumps(value, ensure_ascii=False, indent=2))
...
def goal_evaluate_review(
    ...
    if (week is None) == (month is None):
        _evaluation_error(
            "--week または --month のどちらか一方を指定してください", code=2
        )
    try:
        value = (
            load_weekly_evaluation(_root(root), week or "")
            if week
            else load_monthly_evaluation(_root(root), month or "")
        )
        if not value:
            raise EvaluationError("評価が見つかりません")
...
def goal_evaluate_apply(
    ...
    if not yes and not typer.confirm(
        "評価を承認しますか？目標や計画は変更されません。", default=False
    ):
        _evaluation_error("評価承認を中止しました", code=2)
    try:
        value = approve_evaluation(_root(root), week=week, month=month)
    except (EvaluationError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
...
def _print_replan(value: dict[str, Any]) -> None:
    ...
    typer.echo(f"状態: {value['status']}")
    labels = {
        "reduce_daily_load": "日次負荷削減",
        "extend_deadline": "期限変更",
        "pause_goal": "目標一時停止",
        "remove_blocker": "blocker除去",
        "review_goal_definition": "目標定義見直し",
        "change_minimum": "最低ライン変更",
        "reduce_scope": "スコープ縮小",
    }
    for index, item in enumerate(value.get("proposals") or [], start=1):
        approved = (
            " [承認対象]"
            if item["id"] in value.get("approved_proposal_ids", [])
            else ""
        )
...
def goal_replan(
    ...
    try:
        ...
        if save:
            save_replan(_root(root), value)
    except (ReplanError, EvaluationError, GoalError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
    if json_output:
        typer.echo(json.dumps(value, ensure_ascii=False, indent=2))
        return
...
def goal_replan_review(
    replan_id: str = typer.Argument(...),
    json_output: bool = typer.Option(False, "--json"),
    root: Path | None = RootOption,
) -> None:
    """保存済みreplanの提案と承認対象を表示します。"""
    try:
        value = load_replan(_root(root), replan_id)
...
def goal_replan_edit(
    ...
    setting: str | None = typer.Option(None, "--set"),
    root: Path | None = RootOption,
) -> None:
    """proposalの削除・承認対象追加・安全なafter値の編集を行います。"""
    try:
        value = edit_replan(
            _root(root), replan_id, remove=remove, approve=approve, setting=setting
        )
    except (ReplanError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
...
def goal_replan_apply(
    replan_id: str = typer.Argument(...),
    yes: bool = typer.Option(False, "--yes"),
    root: Path | None = RootOption,
) -> None:
    """承認対象に選んだproposalだけをバックアップ後に適用します。"""
    if not yes and not typer.confirm("承認対象proposalを適用しますか？", default=False):
        _evaluation_error("replan適用を中止しました", code=2)
    try:
        value = apply_replan(_root(root), replan_id)
    except (ReplanError, GoalError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
...
def goal_replan_list(
    json_output: bool = typer.Option(False, "--json"), root: Path | None = RootOption
) -> None:
    """保存済みreplanを一覧表示します。"""
    try:
        values = list_replans(_root(root))
    except (ReplanError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
    if json_output:
        typer.echo(json.dumps({"replans": values}, ensure_ascii=False, indent=2))
        return
    if not values:
        typer.echo("replanはありません")
    for value in values:
        typer.echo(
            f"[{value['status']}] {value['id']}｜{value['source_type']}:{value.get('source_id')}"
        )


@replan_app.command("cancel")
def goal_replan_cancel(
    replan_id: str = typer.Argument(...),
    yes: bool = typer.Option(False, "--yes"),
    root: Path | None = RootOption,
) -> None:
    """未適用replanを取消します。"""
    if not yes and not typer.confirm("replanを取り消しますか？", default=False):
        _evaluation_error("取消を中止しました", code=2)
    try:
        value = cancel_replan(_root(root), replan_id)
    except (ReplanError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
...
def goal_coach(
    week: str | None = typer.Option(None, "--week"),
    month: str | None = typer.Option(None, "--month"),
    copy_prompt: bool = typer.Option(False, "--copy"),
    root: Path | None = RootOption,
) -> None:
    """保存済み評価をChatGPTへ渡すプロンプトを生成します。外部APIは使いません。"""
    try:
        prompt = build_coach_prompt(_root(root), week=week, month=month)
    except (GoalCoachError, EvaluationError, OSError, ValueError) as exc:
        _evaluation_error(str(exc))
    typer.echo(prompt)
    if copy_prompt:
        if not _copy_chat_prompt(prompt):
            _evaluation_error("クリップボードへコピーできません", code=4)
...
def goal_coach_receive(
    ...
    try:
        ...
        _evaluation_error(
            str(exc),
            code=2 if isinstance(exc, (GoalCoachError, typer.BadParameter)) else 3,
        )
...
def input_text(
    ...
    clipboard: bool = typer.Option(
        False, "--clipboard", help="macOSのクリップボードから読み込む"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="保存せずに内容だけを表示する"
    ),
    root: Path | None = RootOption,
) -> None:
...
def _store_natural_input(
    root: Path, day: str, raw_text: str, source: str
) -> tuple[Path, str]:
    """Append one raw input with the same atomic semantics as ``input``."""
    path = inbox_path(root, day)
    try:
        payload = (
            read_json_file(path) if path.exists() else {"date": day, "entries": []}
        )
    except (OSError, ValueError) as exc:
...
def _read_chat_import_input(
    *,
    ...
    if explicit_count > 1 or (explicit_count and piped_text):
        raise ValueError(
            "--json-text、--file、--clipboard、標準入力は同時に使用できません"
        )
...
def _chat_auto_approval_reason(
    root: Path, day: str, draft: dict[str, Any], warnings: list[str]
) -> str | None:
    if warnings:
...
def _print_chat_import_result(
    *,
    ...
    typer.echo(
        "daily-review chat-import｜dry-run"
        if dry_run
        else "ChatGPTの構造化入力を取り込みました"
    )
    typer.echo(f"日付: {day}")
    typer.echo(f"入力元: {source}")
    if input_id:
...
def _copy_chat_prompt(prompt: str) -> bool:
    ...
    try:
        subprocess.run(
            ["pbcopy"], input=prompt, text=True, check=True, capture_output=True
        )
...
def chat_prompt(
    date: str | None = DateOption,
    clipboard: bool = typer.Option(
        False, "--clipboard", help="プロンプトをmacOSのクリップボードへコピーする"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if not path.is_file():
        _input_error(
            f"テンプレートがありません: templates/{CHAT_IMPORT_PROMPT_NAME}\ndaily-review init を実行してください"
        )
    try:
...
def chat_import(
    ...
    file: Path | None = typer.Option(
        None, "--file", help="ChatGPTの出力を保存したUTF-8ファイル"
    ),
    json_text: str | None = typer.Option(
        None, "--json-text", help="ChatGPTのJSONまたはJSONを含む文章"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="検証・変換だけを行い保存しない"
    ),
    approve: bool = typer.Option(
        False, "--approve", help="取込後に既存の確認・修正・承認フローを開始する"
    ),
    yes: bool = typer.Option(
        False, "--yes", help="安全条件を満たす場合のみ確認なしで承認する"
    ),
    output_json: bool = typer.Option(
        False, "--output-json", help="結果をJSONだけで標準出力へ出力する"
    ),
    force: bool = typer.Option(
        False, "--force", help="未承認ドラフトをバックアップして置き換える"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if approve and yes:
        _chat_import_error(
            "--approve と --yes は同時に使用できません", output_json=output_json
        )
    if approve and output_json:
        _chat_import_error(
            "--approve と --output-json は同時に使用できません", output_json=output_json
        )
    base = _root(root)
    ...
    try:
        ...
        _chat_import_error(
            f"既存ドラフトを読み込めません: {exc}", output_json=output_json, code=3
        )
        return
    if existing_draft:
        ...
        if existing_draft.get("import_hash") == content_hash and not force:
            _chat_import_error(
                "同じ内容はすでに取り込み済みです", output_json=output_json
            )
        if not force:
            _chat_import_error(
    ...
    try:
        ...
        _, input_id = _store_natural_input(
            base, day, payload["raw_text"], "chat_import"
        )
        draft = build_chat_import_draft(
            payload, input_id=input_id, content_hash=content_hash, warnings=warnings
        )
        atomic_write_json_data(draft_path(base, day), draft)
    except (OSError, ValueError) as exc:
        _chat_import_error(
            f"構造化入力を保存できません: {exc}", output_json=output_json, code=4
        )
        return
    ...
    if yes:
        ...
        try:
            ...
            _chat_import_error(
                f"承認内容を保存できません: {exc}", output_json=output_json, code=3
            )
            return
        if output_json:
            _chat_import_json_result(
...
def _chat_prompt_for_day(
    base: Path, day: str, summary: dict[str, Any], *, create_priorities: bool
) -> str:
    path = base / "templates" / CHAT_IMPORT_PROMPT_NAME
    if not path.is_file():
...
def _print_chat_completed(base: Path, day: str) -> None:
    ...
    today_main = (
        approval.get("today_main")
        if isinstance(approval.get("today_main"), list)
        else []
    )
    proposal = entry.get("tomorrow_plan_proposal") or {}
    tomorrow_main = (
        proposal.get("main") if isinstance(proposal.get("main"), list) else []
    )
    minimums = [
        task.get("minimum_line")
...
def _chat_resume(base: Path, day: str) -> None:
    ...
    if current and current.get("status") == "approved":
        ...
        _save_chat_session(
            base, day, "draft", draft_path=str(draft_path(base, day).relative_to(base))
        )
...
def _read_chat_paste() -> str:
    typer.echo(
        "JSONを貼り付けてください。最後の行に __END__、または空行を2回入力すると終了します。"
    )
    lines: list[str] = []
    blank_lines = 0
    while True:
...
def chat(
    ...
    resume: bool = typer.Option(
        False, "--resume", help="未承認ドラフトの確認・修正・承認を再開する"
    ),
    prompt_only: bool = typer.Option(
        False, "--prompt-only", help="動的プロンプトだけを表示する"
    ),
    copy_prompt: bool = typer.Option(
        False, "--copy-prompt", help="プロンプトをクリップボードへコピーする"
    ),
    import_only: bool = typer.Option(
        False, "--import-only", help="プロンプトを省略してJSONの取り込みから開始する"
    ),
    clipboard: bool = typer.Option(
        False, "--clipboard", help="macOSのクリップボードからJSONを読み込む"
    ),
    file: Path | None = typer.Option(
        None, "--file", help="ChatGPT出力のUTF-8 JSONファイル"
    ),
    json_text: str | None = typer.Option(
        None, "--json-text", help="ChatGPTのJSONまたはJSONを含む文章"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="JSONを検証・表示するだけで保存しない"
    ),
    yes: bool = typer.Option(
        False, "--yes", help="安全条件を満たす場合のみ確認なしで承認する"
    ),
    ...
    if (
        clipboard or file is not None or json_text is not None or yes or dry_run
    ) and not import_only:
        _input_error(
            "JSON入力オプション、--yes、--dry-run は --import-only と併用してください"
        )
    base = _root(root)
    day = _day(date)
    try:
    ...
    if state == "daily_only":
        typer.echo(
            "ERROR: 日次データは存在しますが、承認状態を確認できません", err=True
        )
    ...
    try:
        ...
        prompt = _chat_prompt_for_day(
            base, day, summary, create_priorities=not prompt_only
        )
    except (OSError, ValueError) as exc:
    ...
    if prompt_only:
        ...
        if copy_prompt:
            if _copy_chat_prompt(prompt):
                ...
                typer.echo(
                    "WARN: コピーできなかったため、上のプロンプトを手動でコピーしてください"
                )
        return
    if state == "draft":
    ...
    should_copy = copy_prompt or typer.confirm(
        "ChatGPT用プロンプトをクリップボードへコピーしますか？", default=True
    )
    if should_copy:
        if _copy_chat_prompt(prompt):
            typer.echo("ChatGPT用プロンプトをコピーしました")
    ...
    typer.echo(
        "[c] クリップボードから読み込む  [p] この画面へ貼り付ける  [f] JSONファイルを指定する  [q] 終了"
    )
    ...
    if source == "c":
        ...
        values = {
            "clipboard": False,
            "file": Path(typer.prompt("JSONファイルのパス")),
            "json_text": None,
        }
    else:
...
def handoff(
    ...
    copy: bool = typer.Option(
        False, "--copy", help="完成したhandoffをmacOSのクリップボードへコピーする"
    ),
    output: Path | None = typer.Option(
        None, "--output", help="handoffを書き出す新規テキストファイル"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if copy:
        if _copy_chat_prompt(package):
            ...
            typer.echo(
                "WARN: コピーできなかったため、上のhandoffを手動でコピーしてください"
            )
    if output:
...
def handoff_list(
    ...
    latest: bool = typer.Option(
        False, "--latest", help="最新の有効handoffだけを表示する"
    ),
    ...
    if json_output:
        typer.echo(
            json.dumps({"date": day, "handoffs": items}, ensure_ascii=False, indent=2)
        )
        return
...
def handoff_cancel(
    ...
    session_id: str | None = typer.Option(
        None, "--session-id", help="キャンセルするhandoffのsession_id"
    ),
    latest: bool = typer.Option(
        False, "--latest", help="最新の未承認handoffをキャンセルする"
    ),
    root: Path | None = RootOption,
) -> None:
...
def receive(
    date: str | None = DateOption,
    clipboard: bool = typer.Option(
        False, "--clipboard", help="macOSのクリップボードからChatGPT回答を読み込む"
    ),
    file: Path | None = typer.Option(None, "--file", help="ChatGPT回答のUTF-8ファイル"),
    json_text: str | None = typer.Option(
        None, "--json-text", help="ChatGPT回答のJSONまたはJSONを含む文章"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="検証・表示だけを行い保存しない"
    ),
    approve: bool = typer.Option(
        False, "--approve", help="受信後に既存の確認・承認フローを開始する"
    ),
    yes: bool = typer.Option(
        False, "--yes", help="安全条件を満たす場合のみ確認なしで承認する"
    ),
    force: bool = typer.Option(
        False, "--force", help="未承認ドラフトまたは受信済みhandoffを安全に再処理する"
    ),
    allow_expired: bool = typer.Option(
        False, "--allow-expired", help="期限切れhandoffの受信を明示的に許可する"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    try:
        content, _ = _read_chat_import_input(
            json_text=json_text, file=file, clipboard=clipboard
        )
        payload, _warnings, manifest, item, handoff_info, content_hash = (
            prepare_receive(
                base,
                content,
        ...
        _handoff_error(
            ...
            day=day_hint,
            code=3,
            next_command="daily-review receive --file response.json"
            if clipboard
            else None,
        )
        return
    ...
    if yes and (item.get("status") != "issued" or is_expired(item)):
        _handoff_error(
            "自動承認できません。handoffが発行済みかつ有効期限内である必要があります",
            day=day,
        )
    if daily_path(base, day).exists():
    ...
    if approve:
        ...
        if refreshed and refreshed.get("status") == "approved":
            update_handoff(
                base, day, manifest, item, status="approved", content_hash=content_hash
            )


def _print_organize_result(
    result: dict[str, Any], base: Path, day: str, *, dry_run: bool
) -> None:
    draft = result["draft"]
    if dry_run:
    ...
    for title, candidates in (
        ("今日のMain候補", draft["today"]["main_candidates"]),
...
def organize(
    date: str | None = DateOption,
    dry_run: bool = typer.Option(
        False, "--dry-run", help="保存せずに整理結果だけを表示する"
    ),
    force: bool = typer.Option(
        False, "--force", help="既存ドラフトを全入力から安全に作り直す"
    ),
    json_output: bool = typer.Option(
        False, "--json", help="整理ドラフトをJSONで表示する"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if not dry_run and result["changed"]:
        try:
            ...
            typer.echo(
                f"ERROR: 整理ドラフトを保存できません: {result['path']} ({exc})",
                err=True,
            )
            raise typer.Exit(code=4) from exc
    if json_output:
...
def review(
    mode: str | None = typer.Argument(
        None, help="quick を指定すると日次レビューを簡単入力"
    ),
    date: str | None = DateOption,
    json_output: bool = typer.Option(
        False, "--json", help="ドラフトJSONをそのまま表示する"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="表示のみでファイルを変更しない"
    ),
    done: list[str] = typer.Option([], "--done", help="今日できたこと。複数指定可"),
    not_done: list[str] = typer.Option(
        [], "--not-done", help="できなかったこと。複数指定可"
    ),
    cause: list[str] = typer.Option([], "--cause", help="崩れた原因。複数指定可"),
    tomorrow: list[str] = typer.Option(
        [], "--tomorrow", help="明日やること。入力順の上位3件をMain候補にする"
    ),
    minimum: list[str] = typer.Option([], "--minimum", help="明日の最低限。複数指定可"),
    journal: str | None = typer.Option(None, "--journal", help="任意の日記（改行可）"),
    stdin: bool = typer.Option(
        False, "--stdin", help="標準入力からクイックレビューJSONを読み込む"
    ),
    force: bool = typer.Option(
        False, "--force", help="同日レビューをバックアップ後に更新"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if mode is not None:
        if mode != "quick":
            ...
            raise typer.Exit(code=2)
        option_used = any(
            (done, not_done, cause, tomorrow, minimum, journal is not None)
        )
        if stdin and option_used:
            typer.echo(
                "ERROR: --stdinと個別入力オプションは同時に指定できません", err=True
            )
            raise typer.Exit(code=2)
        raw_input = ""
        try:
            if stdin:
                ...
                payload = {
                    ...
                    if (
                        value := typer.prompt(
                            "今日できたこと", default="", show_default=False
                        )
                    )
                    else [],
                    "not_done": [value]
                    if (
                        value := typer.prompt(
                            "できなかったこと", default="", show_default=False
                        )
                    )
                    else [],
                    "causes": [value]
                    if (
                        value := typer.prompt(
                            "崩れた原因", default="", show_default=False
                        )
                    )
                    else [],
                    "tomorrow": [value]
                    if (
                        value := typer.prompt(
                            "明日やること", default="", show_default=False
                        )
                    )
                    else [],
                    "minimum": [value]
                    if (
                        value := typer.prompt(
                            "明日の最低限", default="", show_default=False
                        )
                    )
                    else [],
                    "journal": typer.prompt(
                        "日記（任意）", default="", show_default=False
                    ),
                }
                raw_input = json.dumps(payload, ensure_ascii=False, indent=2)
        ...
        if json_output and dry_run:
            typer.echo(
                json.dumps(
                    {"date": day, "dry_run": True, "entry": planned},
                    ensure_ascii=False,
                    indent=2,
                )
            )
            ...
            typer.echo(
                f"完了: {len(normalized['done'])}件 / 未完了: {len(normalized['not_done'])}件"
            )
            typer.echo(
                f"明日のMain候補: {len(planned['tomorrow_plan_proposal']['main'])}件 / 保留: {len(planned['quick_review']['backlog_candidates'])}件"
            )
            typer.echo("保存は行いませんでした")
            return
        try:
            result = save_quick_review(
                base, day, normalized, raw_input=raw_input, force=force
            )
        ...
        if json_output:
            ...
            typer.echo(
                f"明日のMain候補: {len(result['entry']['tomorrow_plan_proposal']['main'])}件"
            )
...
def edit_draft(
    ...
    set_values: list[str] = typer.Option(
        [], "--set", help="field=value。複数指定時はその配列で置換する"
    ),
    force: bool = typer.Option(
        False, "--force", help="承認済みドラフトを編集可能に戻す"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    try:
        replacements = (
            _parse_draft_set_values(set_values)
            if set_values
            else _interactive_draft_replacements(draft)
        )
        if not replacements:
            typer.echo("変更はありません。")
            return
...
def approve(
    ...
    force: bool = typer.Option(
        False, "--force", help="承認済みドラフトをバックアップ後に再承認する"
    ),
    root: Path | None = RootOption,
) -> None:
        ...
        json.dumps(
            {
                ...
                "approved": approved,
                "daily_path": str(daily_file.relative_to(daily_file.parents[2]))
                if daily_file
                else None,
                "dry_run": dry_run,
                "draft": draft,
...
def _reflect_approved_message(day: str, result: dict[str, Path | None]) -> None:
    ...
    typer.echo(f"日付: {day}")
    typer.echo(
        f"保存先: {result['daily_path'].relative_to(result['daily_path'].parents[2])}"
    )
...
def _reflect_interactive(base: Path, day: str, draft: dict[str, Any]) -> bool:
    ...
    typer.echo(
        "ERROR: 編集・確認の回数が上限に達しました。--resumeで再開してください",
        err=True,
    )
    return False
...
def reflect(
    ...
    resume: bool = typer.Option(
        False, "--resume", help="既存の未承認ドラフトから再開する"
    ),
    yes: bool = typer.Option(
        False, "--yes", help="安全条件を満たす場合のみ確認なしで承認する"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="保存せずに入力と整理結果だけを確認する"
    ),
    ...
    if resume:
        if text is not None or clipboard:
            _reflect_error(
                "--resume と --text/--clipboard は同時に使用できません",
        ...
        try:
            ...
            _reflect_error(
                f"整理ドラフトを読み込めません: {exc}", json_output=json_output, code=3
            )
            return
        if not draft:
            _reflect_error(
                "再開できるドラフトがありません\n先に daily-review reflect または daily-review organize を実行してください",
                json_output=json_output,
            )
            return
        if draft.get("status") == "approved":
            if json_output:
                _reflect_json(
                    ...
                    input_id=None,
                    approved=True,
                    daily_file=daily_path(base, day),
                    dry_run=dry_run,
                )
            else:
        ...
        if yes:
            ...
            try:
                ...
                _reflect_error(
                    f"承認内容を保存できません: {exc}", json_output=json_output, code=3
                )
            if json_output:
                _reflect_json(
                    day=day,
    ...
    if existing_draft:
        if existing_draft.get("status") == "approved":
            _reflect_error(
                "この日はすでに確定済みです。修正する場合は既存の編集・再承認フローを使用してください",
                json_output=json_output,
            )
        _reflect_error(
            f"既存ドラフトがあります\ndaily-review reflect --date {day} --resume",
    ...
    if _recent_duplicate_input(base, day, raw_text):
        ...
        if not typer.confirm(
            "同じ内容が直前に保存されています。もう一度保存しますか？", default=False
        ):
            typer.echo("確定せず終了しました")
            return
    try:
    ...
    if yes:
        ...
        if json_output:
            _reflect_json(
                ...
                input_id=input_id,
                approved=True,
                daily_file=approval["daily_path"],
                dry_run=False,
            )
        else:
...
def close_day(
    date: str | None = DateOption,
    file: Path | None = typer.Option(
        None, "--file", help="当日の結果・振り返り・翌日提案の一括JSON"
    ),
    clipboard: bool = typer.Option(
        False, "--clipboard", help="macOSのクリップボードからJSONを読み込む"
    ),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="検証と更新予定の表示だけ行い、保存しない"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    try:
        ...
        day = _resolve_night_date(date, payload)
        entries_by_day, warnings, result_count, result_source_day = _prepare_close_day(
            base, day, payload
        )
    except typer.BadParameter as exc:
    ...
    _print_close_day_summary(
        day, stamped_entries[day], result_count, carryover_count, warnings
    )
...
def approve_plan(
    date: str | None = DateOption,
    force: bool = typer.Option(
        False, "--force", help="既存の確定版を確認なしで上書きする"
    ),
    root: Path | None = RootOption,
) -> None:
...
def today(
    ...
    show_ids: bool = typer.Option(
        False, "--show-ids", help="夜の結果記録用にタスクIDを表示する"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    if pending_source:
        typer.echo(
            f"{target} の指示書は提案版のみです。まだ未承認です（保存元: {pending_source}）。"
        )
        return
...
def record_results(
    ...
    if not entry or not plan or not source_day:
        if _has_pending_by_target(base, target):
            typer.echo(
                f"{target} は提案版のみです。承認後に結果を保存してください。", err=True
            )
        else:
    ...
    entry["task_results"] = _merge_task_results(
        entry.get("task_results") or [], updates
    )
...
def carryover(
    ...
    include_skipped: bool = typer.Option(
        False, "--include-skipped", help="意図的に見送ったタスクも表示する"
    ),
    root: Path | None = RootOption,
) -> None:
    ...
    for task in plan.get("tasks") or []:
        ...
        if status == "skipped" and not include_skipped:
            continue
        if (
            status not in CARRYOVER_STATUSES
            and status is not None
            and not (include_skipped and status == "skipped")
        ):
            continue
        count += 1
...
def show_proposal(
    ...
    if not entry or not entry.get("tomorrow_plan_proposal"):
        ...
        raise typer.Exit(code=1)
    _print_plan_summary(
        "明日の指示書・提案版", entry["tomorrow_plan_proposal"], "未承認"
    )
    typer.echo("承認する場合:")
    typer.echo(f"daily-review approve-plan --date {day}")
...
def _print_summary(
    summary: dict[str, Any], *, title: str = "状況", next_override: str | None = None
) -> None:
    ...
    if summary["today_main"]:
        ...
        typer.echo("未記録")
    result_label = (
        f"{results['recorded']}/{results['total']}件"
        if today_final or results["total"]
        else "未記録"
    )
    typer.echo(f"タスク結果: {result_label}")
    typer.echo(
        f"夜の振り返り: {'記録済み' if summary['night_review_exists'] else '未記録'}"
    )
...
def summary(
    ...
    if plan:
        ...
        typer.echo(
            f"目標進捗反映: {'未実行' if plan.get('goal_links') else '対象なし'}"
        )
    week_start, _ = week_range_for(report["date"])
    try:
        weekly_evaluation = load_weekly_evaluation(_root(root), week_start)
    except (EvaluationError, OSError, ValueError):
        weekly_evaluation = None
    typer.echo(
        f"週次目標評価: {'承認済み' if weekly_evaluation and weekly_evaluation.get('status') == 'approved' else 'ドラフト' if weekly_evaluation else '未作成'}"
    )
    if report["errors"]:
...
def home(
    ...
    try:
        morning = run_operational_flow(
            base,
            "morning",
            day=report["date"],
            current=_scheduler_now(base, None),
            dry_run=True,
        )["details"]
    except (OSError, ValueError, SchedulerError):
        morning = {
            ...
            "overdue": [],
            "previous_incomplete": [],
            "warnings": ["scheduler設定または朝の状態を確認できません"],
        }
    try:
        scheduler_state = scheduler_status(base, _scheduler_now(base, None))
    except (OSError, ValueError, SchedulerError) as exc:
        scheduler_state = {"status": "error", "error": str(exc)}
    backups = list_backups(base)
    last_backup = backups[0] if backups else None
    doctor_report = run_doctor(base)
    doctor_errors = [
        item for item in doctor_report["issues"] if item["level"] == "ERROR"
    ]
    if output_format == "json":
        typer.echo(
            json.dumps(
                {
                    "date": report["date"],
                    "instruction": report["today_final"],
                    "main": report["today_main"][:3],
                    "minimum": morning["minimum"],
                    "task_results": report["task_results"],
                    "overdue": morning["overdue"],
                    "previous_incomplete": morning["previous_incomplete"],
                    "night_review_recorded": report["night_review_exists"],
                    "tomorrow_instruction_status": (
                        "approved"
                        if report["tomorrow_final"]
                        else "draft"
                        if report["tomorrow_proposal"]
                        else "missing"
                    ),
                    "week_recorded_days": report["week_recorded_days"],
                    "scheduler": scheduler_state,
                    "last_backup": last_backup,
                    "doctor_error_count": len(doctor_errors),
                    "next_action": home_next_command(report),
    ...
    try:
        designs = []
        for path in (
            sorted((base / "data" / "goal-designs").glob("design-*.json"))
            if (base / "data" / "goal-designs").is_dir()
            else []
        ):
        ...
        if not resume_next:
            draft_replans = [
                item for item in list_replans(base) if item.get("status") == "draft"
            ]
            if draft_replans:
                latest_replan = max(
                    draft_replans,
                ...
                action = (
                    "apply" if latest_replan.get("approved_proposal_ids") else "review"
                )
    ...
    handoff_state = "none"
    handoff_eligible = (
        not report.get("draft")
        and not report.get("entry")
        and not report.get("inbox_entry_count")
    )
    if handoff_eligible:
        try:
    ...
    if "--import-only --clipboard" in chat_next and handoff_state == "none":
        ...
        handoff_next = (
            f"daily-review handoff --date {report['date']} --copy"
            if handoff_state in {"none", "expired"}
            else ""
        )
    planning_next = ""
    try:
        active_goals = [
            goal for goal in load_goals(base) if goal.get("status") == "active"
        ]
        week_start, _ = week_range_for(report["date"])
    ...
    _print_summary(
        report,
        title="daily-review home",
        next_override=resume_next
        or planning_next
        or (
            handoff_next if handoff_eligible else chat_next or home_next_command(report)
        ),
    )
    ...
    if report["incomplete_tasks"]:
        for index, item in enumerate(report["incomplete_tasks"], start=1):
            task = item["task"]
            typer.echo(
                f"{index}. [{task.get('area', '未設定')}] {task.get('task', '未設定')}（{_task_result_label(item['status'])}）"
            )
    else:
        typer.echo("なし")
    if report["tomorrow_final"]:
        ...
        if not goals["active_count"]:
            ...
            if goals["near_due"]:
                ...
                for goal in goals["near_due"]:
                    remaining = (
                        parse_date(goal["due_date"]) - parse_date(report["date"])
                    ).days
            ...
            if next_actions:
                ...
                for goal, action in next_actions[:2]:
                    ...
                    typer.echo(
                        f"- {goal.get('category') or goal['title']}｜{item['title']}"
                    )
    ...
    if report["night_review_exists"] and report["date"] == week_end:
        try:
            weekly_evaluation = load_weekly_evaluation(base, week_start)
        except (EvaluationError, OSError, ValueError):
            weekly_evaluation = None
        if not weekly_evaluation:
            typer.echo(
                f"目標評価の次操作: daily-review goal evaluate week --date {report['date']} --save"
            )
        elif weekly_evaluation.get("status") != "approved":
            typer.echo(
                f"目標評価の次操作: daily-review goal evaluate review --week {week_start}"
            )
    if report["night_review_exists"] and report["date"] == month_end:
        try:
            monthly_evaluation = load_monthly_evaluation(base, month_start[:7])
        except (EvaluationError, OSError, ValueError):
            monthly_evaluation = None
        if not monthly_evaluation:
            typer.echo(
                f"月次評価の次操作: daily-review goal evaluate month --month {month_start[:7]} --save"
            )
    typer.echo("最低限:")
    if morning["minimum"]:
        for item in morning["minimum"][:3]:
            typer.echo(f"- {item['minimum_line']}")
    else:
        typer.echo("未記録")
    typer.echo(f"期限超過: {len(morning['overdue'])}件")
    typer.echo(f"前日からの未完了: {len(morning['previous_incomplete'])}件")
    typer.echo(
        f"scheduler: {'有効' if scheduler_state.get('enabled') else '無効または未設定'}"
    )
    typer.echo(
        "最後のbackup: "
        + (
            str(last_backup.get("created_at") or last_backup.get("file_name"))
            if last_backup
            else "未作成"
        )
    )
    if doctor_errors:
        typer.echo(
            f"WARN: doctorで重大エラーが{len(doctor_errors)}件あります。daily-review doctor を実行してください。"
        )
    if report["errors"]:
        for error in report["errors"]:
            typer.echo(f"ERROR: {error}", err=True)
...
def status(
    ...
    typer.echo(f"生ログ        {_saved_label(entry.get('raw_log'))}")
    typer.echo(
        f"整形ログ      {_saved_label(entry.get('structured_review') or entry.get('diary'))}"
    )
        ...
        typer.echo(
            " | ".join(
                [
                    ...
                    "保存済み"
                    if entry.get("structured_review") or entry.get("diary")
                    else "未保存",
                    "保存済み" if entry.get("tomorrow_plan_proposal") else "未保存",
                    "承認済み"
                    if (entry.get("tomorrow_plan_final") or {}).get("status")
                    == "approved"
                    else "未承認",
...
def weekly(
    ...
    root: Path | None = RootOption,
    dry_run: bool = typer.Option(
        False, "--dry-run", help="保存せず集計だけ確認します。"
    ),
    approve: bool = typer.Option(
        False, "--approve", help="保存済みdraftを承認します。"
    ),
    yes: bool = typer.Option(False, "--yes", help="承認確認を省略します。"),
    force: bool = typer.Option(
        False, "--force", help="stale draftを明示的に再生成します。"
    ),
    output_format: str = typer.Option("text", "--format", help="text または json。"),
) -> None:
    ...
    if approve:
        if dry_run:
            raise typer.BadParameter("--approveと--dry-runは同時に指定できません")
        if not yes and not typer.confirm(
            "この週次レポートを承認しますか？", default=False
        ):
            typer.echo("承認を中止しました")
            raise typer.Exit(code=2)
        try:
            result = approve_report(base, period_type="weekly", target=day)
        except (OSError, ValueError) as exc:
            typer.echo(f"ERROR: 週次レポートを承認できません: {exc}", err=True)
            raise typer.Exit(code=3) from exc
        typer.echo(
            json.dumps(result, ensure_ascii=False, indent=2)
            if output_format == "json"
            else f"週次レポートを承認しました: {result['path']}"
        )
        return
    ...
    summary.update(
        {
            "status": "draft",
            "source_revision": report_source_revision(base, start, end),
            "generated_at": now_iso(),
            "approved_at": None,
        }
    )
    json_path = weekly_path(base, start, end)
    existing = read_json_file(json_path) if json_path.exists() else None
    stale = (
        isinstance(existing, dict)
        and existing.get("source_revision") != summary["source_revision"]
    )
    if stale and not force:
        typer.echo(
            "ERROR: 保存済み週次レポートがstaleです。確認後に--forceで再生成してください",
            err=True,
        )
        raise typer.Exit(code=3)
    if dry_run:
        if output_format == "json":
            typer.echo(
                json.dumps(
                    {"status": "dry_run", "report": summary},
                    ensure_ascii=False,
                    indent=2,
                )
            )
        else:
            typer.echo("週次集計｜dry-run")
            typer.echo(f"対象期間: {start}〜{end}")
            typer.echo("保存は行いませんでした")
        return
    write_text(json_path, json.dumps(summary, ensure_ascii=False, indent=2) + "\n")
    markdown_path = write_text(
        weekly_log_path(base, start, end), render_weekly(summary)
    )
    if output_format == "json":
        typer.echo(
            json.dumps(
                {"status": "saved", "path": str(json_path), "report": summary},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    typer.echo(
        f"Main: 完了 {main['completed']}/{main['recorded']}（{'算出不可' if main['percent'] is None else str(main['percent']) + '%'}）、結果未記録 {main['unrecorded']}件"
    )
    minimum = summary["minimum_line_summary"]
    typer.echo(
        f"最低ライン達成率: {'算出不可' if minimum['percent'] is None else str(minimum['percent']) + '%'}"
    )
    continuity = summary["continuity"]
    typer.echo(
        f"継続状況: 振り返り {continuity['review_recorded']['count']}/{continuity['review_recorded']['total']}日、結果 {continuity['task_results_recorded']['count']}/{continuity['task_results_recorded']['total']}日、確定版 {continuity['approved_plan']['count']}/{continuity['approved_plan']['total']}日"
    )
    typer.echo(
        f"崩れた原因: {summary['failure_reasons'][0]['cause'] if summary['failure_reasons'] else 'なし'}"
    )
    typer.echo(
        f"引き継ぎが多いタスク: {summary['carryover_analysis'][0]['task'] if summary['carryover_analysis'] else 'なし'}"
    )
    ...
    if task_summary["total"]:
        ...
        typer.echo(
            f"通常タスク完了率: {completion['percent']}%（{completion['completed']}/{completion['total']}）"
        )
        typer.echo(
            f"最低ライン達成率: {task_minimum['percent']}%（{task_minimum['achieved']}/{task_minimum['total']}）"
        )
    else:
        typer.echo("集計対象なし")
    typer.echo(f"承認: daily-review weekly --date {day} --approve")
...
def _backup_root(ctx: typer.Context, root: Path | None) -> Path:
    group_root = ctx.obj.get("backup_root") if isinstance(ctx.obj, dict) else None
    return _root(root if root is not None else group_root)
...
def backup_list_command(
    ...
    values = list_backups(_backup_root(ctx, root), directory)
    if output_format == "json":
...
def release_check(
    ...
    """package・安全機能・文書・Gitを読み取り専用でv1.3向けに確認します。"""
    ...
    if json_output:
        ...
        typer.echo(f"daily-review release-check v{__version__}")
        for item in report["checks"]:
            detail = f": {item['detail']}" if item["detail"] else ""
            typer.echo(f"{item['level']:<5}{item['name']}{detail}")
        typer.echo(
            "daily-review release-check: OK"
            if report["result"] == "READY"
            else "daily-review release-check: ERROR"
        )
...
def _scheduler_now(base: Path, at: str | None) -> datetime:
    return parse_scheduler_at(at, load_scheduler_config(base)["timezone"])


SchedulerResult = TypeVar("SchedulerResult")


def _scheduler_guard(operation: Callable[[], SchedulerResult]) -> SchedulerResult:
    try:
        return operation()
        ...
        typer.echo(f"ERROR: ファイル操作に失敗しました: {exc}", err=True)
        raise typer.Exit(code=4) from exc


def _structured_output(
    title: str, value: dict[str, Any] | list[Any], output_format: str
) -> None:
    if output_format not in {"human", "json"}:
        raise typer.BadParameter("--formatはhumanまたはjsonにしてください")
...
def scheduler_status_command(
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """有効状態、次回job、失敗、lock、launchd状態を表示します。"""
    base = _root(root)
    _structured_output(
        "daily-review scheduler status",
        _scheduler_guard(lambda: scheduler_status(base, _scheduler_now(base, at))),
        output_format,
    )
...
def scheduler_list_command(
    ...
    """設定済みjob、schedule、retry方針を一覧表示します。"""
    base = _root(root)
    values = _scheduler_guard(
        lambda: [
            {
                "id": item.id,
            ...
            for item in configured_jobs(base)
        ]
    )
    _structured_output("daily-review scheduler list", values, output_format)


@scheduler_app.command("due")
def scheduler_due_command(
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """grace、missed run、retry、成功済みslotを考慮してdueを判定します。"""
    base = _root(root)
    _structured_output(
        "daily-review scheduler due",
        _scheduler_guard(lambda: due_jobs(base, _scheduler_now(base, at))),
        output_format,
    )


@scheduler_app.command("run-due")
def scheduler_run_due_command(
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="履歴・通知・データを書き込みません。"
    ),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """dueなjobを一度ずつ実行し、retryは次のpollに委ねます。"""
    base = _root(root)
    _structured_output(
        "daily-review scheduler run-due",
        _scheduler_guard(
            lambda: run_due_jobs(base, _scheduler_now(base, at), dry_run=dry_run)
        ),
        output_format,
    )
...
def scheduler_run_command(
    job: str = typer.Argument(..., help="実行するjob ID。"),
    date: str | None = DateOption,
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    dry_run: bool = typer.Option(False, "--dry-run", help="一切書き込みません。"),
    force: bool = typer.Option(
        False, "--force", help="成功済みslotを手動再実行します。"
    ),
    idempotency_key: str | None = typer.Option(None, "--idempotency-key"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """jobを手動実行します。forceでも承認やrollover applyは行いません。"""
    base = _root(root)
    current = _scheduler_guard(lambda: _scheduler_now(base, at))
    planned = None
    if date:
        day = _day(date)
        selected = _scheduler_guard(
            lambda: next(
                (item for item in configured_jobs(base) if item.id == job), None
            )
        )
        if selected is None:
            raise typer.BadParameter(f"jobが見つかりません: {job}")
        job_time = datetime.strptime(selected.schedule["time"], "%H:%M").time()
        planned = datetime.combine(parse_date(day), job_time, current.tzinfo)
    _structured_output(
        "daily-review scheduler run",
        _scheduler_guard(
            lambda: run_scheduled_job(
                base,
                job,
                current=current,
                planned=planned,
...
def scheduler_history_command(
    ...
    """job履歴をjob、status、日付で絞り込みます。"""
    base = _root(root)
    values = _scheduler_guard(
        lambda: scheduler_history(base, job_id=job, status=status, day=date)
    )
    _structured_output("daily-review scheduler history", values, output_format)


@scheduler_app.command("doctor")
def scheduler_doctor_command(
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at", help="テスト用のISO 8601現在時刻。"),
    repair: bool = typer.Option(
        False, "--repair", help="stale lockだけを安全に削除します。"
    ),
    dry_run: bool = typer.Option(False, "--dry-run", help="修復予定だけを表示します。"),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """設定、履歴、lock、通知sender、launchdを診断します。"""
    base = _root(root)
    result = _scheduler_guard(
        lambda: scheduler_doctor(
            base, _scheduler_now(base, at), repair=repair, dry_run=dry_run
        )
    )
    _structured_output("daily-review scheduler doctor", result, output_format)
    if result["status"] in {"error", "critical"}:
        raise typer.Exit(code=1)


@scheduler_app.command("install")
def scheduler_install_command(
    root: Path | None = RootOption,
    backend: str = typer.Option("launchd", "--backend", help="launchd または cron。"),
    dry_run: bool = typer.Option(
        False, "--dry-run", help="登録せず内容だけ表示します。"
    ),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """launchdへpoll jobを明示的に導入します。cronは例だけを返します。"""
    base = _root(root)
    _structured_output(
        "daily-review scheduler install",
        _scheduler_guard(
            lambda: install_scheduler(base, backend=backend, dry_run=dry_run)
        ),
        output_format,
    )
...
def scheduler_uninstall_command(
    dry_run: bool = typer.Option(
        False, "--dry-run", help="削除せず予定だけ表示します。"
    ),
    output_format: str = typer.Option("human", "--format", help="human または json。"),
) -> None:
    """launchd登録だけを外します。daily-reviewデータと履歴は削除しません。"""
    _structured_output(
        "daily-review scheduler uninstall",
        _scheduler_guard(lambda: uninstall_scheduler(dry_run=dry_run)),
        output_format,
    )


@scheduler_app.command("cron-example")
def scheduler_cron_example_command(root: Path | None = RootOption) -> None:
    """crontabを変更せず、portableな登録例だけを表示します。"""
    typer.echo(_scheduler_guard(lambda: cron_example(_root(root))))


def _run_flow_command(
    kind: str,
    *,
    root: Path | None,
    date: str | None,
    ...
    base = _root(root)
    result = _scheduler_guard(
        lambda: run_operational_flow(
            base,
            kind,
            day=_day(date) if date else None,
            month=month,
            current=_scheduler_now(base, at),
            dry_run=dry_run,
            force=force,
        )
    )
    _structured_output(f"daily-review flow {kind}", result, output_format)


@flow_app.command("morning")
def flow_morning_command(
    date: str | None = DateOption,
    root: Path | None = RootOption,
    ...
    """今日の確定版、Main最大3件、最低限、期限超過を確認します。"""
    _run_flow_command(
        "morning",
        root=root,
        date=date,
...
@flow_app.command("nightly")
def flow_nightly_command(
    date: str | None = DateOption,
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at"),
    dry_run: bool = typer.Option(False, "--dry-run"),
    ...
    """close-day後のdraft、通知、rollover preview、整合性確認を実行します。"""
    _run_flow_command(
        "nightly",
        root=root,
        date=date,
...
@flow_app.command("weekly")
def flow_weekly_command(
    date: str | None = DateOption,
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at"),
    dry_run: bool = typer.Option(False, "--dry-run"),
...
@flow_app.command("monthly")
def flow_monthly_command(
    month: str | None = typer.Option(None, "--month", help="対象月（YYYY-MM）。"),
    root: Path | None = RootOption,
    at: str | None = typer.Option(None, "--at"),
    dry_run: bool = typer.Option(False, "--dry-run"),
    force: bool = typer.Option(False, "--force"),
    ...
    """月次draft、前月比較、次月重点候補、backupを生成します。"""
    _run_flow_command(
        "monthly",
        root=root,
        date=None,
...
def monthly(
    ...
    if approve:
        if dry_run:
            raise typer.BadParameter("--approveと--dry-runは同時に指定できません")
        if not yes and not typer.confirm(
            "この月次レポートを承認しますか？", default=False
        ):
            typer.echo("承認を中止しました")
            raise typer.Exit(code=2)
        try:
            result = approve_report(base, period_type="monthly", target=month)
        except (OSError, ValueError) as exc:
            typer.echo(f"ERROR: 月次レポートを承認できません: {exc}", err=True)
            raise typer.Exit(code=3) from exc
        typer.echo(
            json.dumps(result, ensure_ascii=False, indent=2)
            if output_format == "json"
            else f"月次レポートを承認しました: {result['path']}"
        )
        return
    effective_end = min(end, today_string()) if start <= today_string() <= end else end
    summary = build_report(base, start, effective_end, period_type="monthly")
    summary["weekly_trends"] = weekly_trends(base, start, effective_end)
    previous_end = (parse_date(start) - timedelta(days=1)).isoformat()
    previous_start, previous_end = month_range_for(previous_end)
    previous = build_report(base, previous_start, previous_end, period_type="monthly")
    summary.update(
        {
            "status": "draft",
            "source_revision": report_source_revision(base, start, effective_end),
            "generated_at": now_iso(),
            "approved_at": None,
            ...
            "next_month_focus_candidate": summary["improvement_suggestion"]["text"],
        }
    )
    ...
    if stale and not force:
        typer.echo(
            "ERROR: 保存済み月次レポートがstaleです。確認後に--forceで再生成してください",
            err=True,
        )
        raise typer.Exit(code=3)
    if dry_run:
        if output_format == "json":
            ...
            typer.echo("月次集計｜dry-run")
            typer.echo(f"対象月: {start}〜{effective_end}")
            typer.echo("保存は行いませんでした")
        return
    write_text(json_path, json.dumps(summary, ensure_ascii=False, indent=2) + "\n")
    ...
    typer.echo(
        f"Main達成率: {'算出不可' if summary['main_summary']['percent'] is None else str(summary['main_summary']['percent']) + '%'}"
    )
    minimum = summary["minimum_line_summary"]
    typer.echo(
        f"最低ライン達成率: {'算出不可' if minimum['percent'] is None else str(minimum['percent']) + '%'}"
    ...
    typer.echo(f"承認: daily-review monthly --date {day} --approve")
