...
def workflow_state(root: Path, day: str, draft: dict[str, Any] | None) -> str:
    ...
    if daily_exists:
        return (
            "approved" if draft and draft.get("status") == "approved" else "daily_only"
        )
    if draft:
...
def build_dynamic_prompt(
    root: Path, day: str, summary: dict[str, Any], template: str, priorities: list[str]
) -> str:
    ...
    if daily_goal_plan and daily_goal_plan.get("status") == "approved":
        ...
        for index, item in enumerate(
            daily_goal_plan.get("main_candidates") or [], start=1
        ):
            lines.append(f"{index}. {item['title']}")
        plan_minimums = [
            item.get("minimum")
            for item in daily_goal_plan.get("main_candidates") or []
            if item.get("minimum")
        ]
        if plan_minimums:
            lines.append("最低ライン:")
