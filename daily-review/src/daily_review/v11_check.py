...
def collect_v11_checks(root: Path) -> dict[str, Any]:
    ...
    checks.append(
        _check(
            "package version v1.1 compatible",
            __version__ in {"1.1.0", "1.2.0rc1", "1.2.0", "1.3.0"},
    ...
    checks.append(
        _check(
            "priorities config",
            _priorities_are_valid(config),
            "config/priorities.json がないか不正です",
        )
    )
    ...
    checks.append(
        _check(
            "chat import prompt",
            prompt.is_file(),
            "templates/chat_import_prompt.md がありません",
        )
    )
    checks.append(
        _check("chat schema", SCHEMA_VERSION == "1.0", f"chat schema: {SCHEMA_VERSION}")
    )
    checks.append(
        _check(
            "handoff schema",
            HANDOFF_VERSION == "1.0",
            f"handoff schema: {HANDOFF_VERSION}",
        )
    )
    ...
    checks.append(
        _check(
            ...
            all(
                (root / item).is_dir()
                for item in ("data/backups/daily", "data/backups/drafts")
            ),
            "backup directories がありません",
        )
    )
    ...
    try:
        ...
        ignored = (
            "data/" in contents
            and "logs/" in contents
            and "config/priorities.json" in contents
        )
    except OSError:
        pass
    checks.append(
        _check(
            ...
            ignored,
            "data/、logs/、config/priorities.json のGit除外を確認できません",
        )
    )

    for name in (
        "natural input workflow",
        "draft review workflow",
        "approval workflow",
        "ChatGPT import workflow",
        "handoff and receive workflow",
        "resume workflow",
    ):
    ...
    checks.append(
        _check(
            "inbox data",
            not any("inbox" in item for item in doctor_errors),
            "; ".join(doctor_errors),
        )
    )
    checks.append(
        _check(
            "drafts data",
            not any("draft" in item for item in doctor_errors),
            "; ".join(doctor_errors),
        )
    )
    checks.append(
        _check(
            "sessions data",
            not any("session" in item for item in doctor_errors),
            "; ".join(doctor_errors),
        )
    )
    checks.append(
        _check(
            "handoffs data",
            not any("handoff" in item for item in doctor_errors),
            "; ".join(doctor_errors),
        )
    )
    checks.append(
        _check(
            "approved daily links",
            not any("approved" in item for item in doctor_errors),
            "; ".join(doctor_errors),
        )
    )
    ...
    try:
        ...
        migration_ok = False
    checks.append(
        _check("migration history", migration_ok, "v1.1-base の移行履歴がありません")
    )
