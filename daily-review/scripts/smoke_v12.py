...
def run(root: Path, *args: str) -> str:
    ...
    if result.exit_code != 0:
        raise RuntimeError(f"{' '.join(args)} failed ({result.exit_code})\n{result.output}")
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="daily-review-v12-smoke-") as directory:
        ...
        run(root, "goal", "design", "answer", design_id, "--answer", "2026-08-31までに過去問5年分を解ける")
        proposal = {
            "goal": {"title": "大学院入試に合格する", "level": "medium", "category": "院試", "due_date": "2026-08-31", "qualitative": ["解法を説明できる"]},
            "milestones": [{"title": "過去問一周", "due_date": "2026-07-31", "steps": [{"title": "2025年度を解く", "minimum": "問題文を読む"}]}],
        }
        run(root, "goal", "design", "receive", design_id, "--json-text", json.dumps(proposal, ensure_ascii=False))
        ...
        goal = json.loads((root / "data/goals/items" / f"{goal_id}.json").read_text(encoding="utf-8"))
        ...
        for offset in range(7):
            ...
            daily = {
                "date": day,
                "structured_review": {
                    "today_main": [{"area": "院試", "status": "完了" if offset == 0 else "一部進んだ"}],
                    "minimum_line": {"院試": "達成"},
                    "what_went_well": ["継続した"],
                    "breakdown_causes": [],
                },
            }
            (root / "data/daily" / f"{day}.json").write_text(json.dumps(daily, ensure_ascii=False), encoding="utf-8")
        run(root, "goal", "link", "--date", "2026-07-14", "--main-index", "1", "--goal", goal_id, "--milestone", milestone_id, "--step", step_id)
        ...
        created = json.loads(run(root, "goal", "replan", "--week", "2026-07-14", "--save", "--json"))
        run(root, "goal", "replan", "edit", created["id"], "--approve", created["proposals"][0]["id"])
